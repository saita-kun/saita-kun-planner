...
schema 準拠を完全検証する専用チェッカーは、現時点ではこのキットに同梱していません。代わりに、`schemas/application-record.schema.json` から `required`、`result` enum、`lessons[].phase` enum、`additionalProperties=false` を読み取って照合する次の検査を実行してください（python3 標準ライブラリだけで動きます）。**受入基準は終了コードが 0 になることです。** exit 0 になるまで、この record を完成扱いにしないでください。
...
def check(obj, node, path):
    ...
    for key, value in obj.items():
        ...
        if "enum" in rule and value not in rule["enum"]:
            errors.append(f"{path}.{key}: {value!r} is not one of {rule['enum']}")
        if rule.get("type") == "array" and isinstance(value, list):
            item = rule.get("items", {})
            if isinstance(item, dict) and item.get("type") == "object":
                for index, element in enumerate(value):
                    if isinstance(element, dict):
                        check(element, item, f"{path}.{key}[{index}]")
