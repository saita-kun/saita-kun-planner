...
schema 準拠を完全検証する専用チェッカーは、現時点ではこのキットに同梱していません。代わりに、`schemas/application-record.schema.json` から `type`（null を含む複数型）、`items`、`pattern`、`required`、`enum`、`additionalProperties=false` を読み取り、配列要素や lessons 内も照合する次の検査を実行してください（python3 標準ライブラリだけで動きます）。**受入基準は終了コードが 0 になることです。** exit 0 になるまで、この record を完成扱いにしないでください。

```bash
python3 - knowledge/records/<subsidy_id>-<round>.json <<'PY'
import json
import pathlib
import re
import sys

schema = json.loads(pathlib.Path("schemas/application-record.schema.json").read_text(encoding="utf-8"))
...
TYPE_CHECKS = {
    ...
    # JSON Schema integers include numbers with a zero fractional part, but not booleans.
    "integer": lambda value: type(value) is int or (type(value) is float and value.is_integer()),
...
def check(value, node, path):
    ...
    if expected is not None:
        ...
        if not any(TYPE_CHECKS[kind](value) for kind in types):
            errors.append(f"{path}: expected type {'/'.join(types)}")
            return
    if "enum" in node and value not in node["enum"]:
        errors.append(f"{path}: {value!r} is not one of {node['enum']}")
    # Record patterns are anchored; fullmatch also rejects a trailing newline.
    ...
    if isinstance(value, dict):
        ...
        for key in node.get("required", []):
            if key not in value:
                errors.append(f"{path}.{key}: missing required key: {key}")
        if node.get("additionalProperties") is False:
            for key in value:
                if key not in properties:
                    errors.append(f"{path}.{key}: unexpected key: {key}")
        for key, rule in properties.items():
            if key in value:
                check(value[key], rule, f"{path}.{key}")
    elif isinstance(value, list):
        for index, element in enumerate(value):
            check(element, node.get("items", {}), f"{path}[{index}]")
...
if errors:
    for error in errors:
        print(f"FAIL: {error}")
    sys.exit(1)
print("OK: record matches application-record schema")
PY
```

FAIL 行が出た場合は record を直してから再実行してください。専用チェッカー（`tools/check-record.sh`）の同梱は、canonical repo（`saita-kun/saita-kun-planner`）の Issue で提案してください。
