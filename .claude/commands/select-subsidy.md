...
原本ファイルを入手したら、顧客環境で SHA-256 を計算して `source_documents[].sha256` と照合してもらってください。計算手段は顧客環境で使えるものを選びます。このキットの必須要件である Python 3 なら標準ライブラリだけで計算できます。
...
選んだ bundled spec に対して、必ず次を実行してください。
...
コマンド自体を実行できなかった場合（Python 3 が見つからない等）は「未実行」であり、green と同一視しないでください。その場合も `input/current-application.json` を作成・更新せず、環境不備として `/setup` の環境セルフチェックへ戻してください。

### 5. current-application を spec_confirmed で初期化する

顧客本人が「この同梱 spec を今回の対象として使う」と確認し、check-spec の終了コードが 0（green）であれば、`input/current-application.json` を作成または更新してください。未実行のまま作成しないでください。
