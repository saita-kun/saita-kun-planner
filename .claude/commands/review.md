...
3. `input/current-application.json` の `spec_path` が non-null で、解決順で選んだ confirmed spec JSON が実在すること。
   ...
   - 解決順で選んだ spec に対して `bash tools/check-spec.sh <解決済みspec>` を**必ず実行し、終了コードが 0**（green）であることを確認してください。`status=confirmed` のまま clause だけが後から編集された stale な spec は、ここで `confirmation spec_sha256 mismatch` として検出されます。**この確認は後段の `bash tools/check-quotes.sh` の前提です**（引用照合は `clauses[].text` との一致だけを見るため、spec 自体が原本と突合済みかは見ません）。
   - FAIL が出た場合はレビューへ進まず、`/confirm-spec` で再突合してください。コマンド自体を実行できなかった場合（Python 3 が見つからない等）は「未実行」であり、green と同一視しないでください。環境不備として `/setup` の環境セルフチェックへ戻してください。
...
## clause 引用の機械照合（必須ゲート）

`quoted_text` が本当に `clauses[].text` にあるかは、目視の自己申告で判定しないでください。clause-verifier チェックの引用表を書いてレビュー結果を保存したら、**必ず**次を実行します。

```bash
bash tools/check-quotes.sh <spec> input/reviews/<レビューファイル>.md
```

- `<spec>` は「spec / confirmation / notes の解決順」で選んだ confirmed spec のパスです。**前提チェックで同じ spec の `bash tools/check-spec.sh` が exit 0 であること**を先に確認してください。このチェッカーは `clauses[].text` との一致だけを見るため、spec 自体が原本と突合済みかは検証しません。
- **受入基準は終了コードが 0 であること**です。0 以外の状態で「引用は完全一致でした」と報告しないでください。
- `FAIL:` 行が指す引用は `clauses[].text` から確認できていません。その根拠は使わず、捏造リスクとして「重大度別の指摘」の**高**に分類してください。`unknown clause_id` の FAIL も同じ扱いです。
- `WARN:` 行には 2 種類あります。取り違えないでください。
  - `... matches clauses[].text only after whitespace normalization` — 空白の有無だけが原本と違う引用です。捏造リスクではありませんが、`quoted_text` を原本どおりに直してから再実行してください。
  - `no clause quotation rows found in ...` — **引用が 1 件も照合されていません**（引用表が空、または全行が `-` などの空欄）。この WARN が出た実行は「引用を検証した」ことになりません。募集要項に基づく指摘があるのに表が空なら、`clause_id` と `quoted_text` を埋めて再実行してください。指摘が本当に顧客資料だけに基づく場合は、レビュー結果に「募集要項に基づく指摘なし（引用照合 0 件）」と明記し、exit 0 を「引用は照合済み」と報告しないでください。
- コマンドを実行できなかった場合は「未実行」であり、green と同一視しないでください。実行できない場合はレビュー結果を green と報告せず、`/setup` の環境セルフチェックへ戻してください。
...
- 実行コマンド: `bash tools/check-quotes.sh <spec> input/reviews/<レビューファイル>.md`
- 終了コード: <0 または 0 以外>
- FAIL 行: <あれば全文をそのまま転記し、「重大度別の指摘」の高へ反映>
- WARN 行: <あれば全文をそのまま転記>
...
- 引用表を作ったら `bash tools/check-quotes.sh <spec> input/reviews/<レビューファイル>.md` を実行し、終了コード 0 を確認してから報告してください。FAIL 行は捏造リスクに分類します。
