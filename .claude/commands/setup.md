...
4. **result-report 任意提出の確認**
   ...
   - 選択肢は「提出する / 提出しない / 保留」の 3 つです。その場で決められない場合は「保留」で構いません。**「保留」は今回は提出しない扱い**で、`/setup` を再実行すれば後から変更できます。

## 完了状態の記録（setup-state）

すべての確認項目が green で、利用者本人の利用規約への同意を確認した後にのみ、`input/setup-state.json` を書き込んでください。同意がない・確認が未完了の状態では書き込みません。

**利用者本人から回答を得られない非対話実行（自動実行、CI、AI 単独実行など、本人がその場にいない実行）では、`input/setup-state.json` を書き込まないでください。** 既定値で埋めて書き込むと、本人が同意していない規約に同意した記録が残ります（設計不変条件 [DR-002](../../docs/design/decisions/dr-002-setup-gate-entry-unification.md)「setup-state を AI が自動生成して同意をスキップしない」）。この場合は `/setup` を未完了のまま終え、そのことを実行ログに残してください。後続の slash command は `CLAUDE.md` の共通不変条件（setup ゲート）で `/setup` 案内に落ちるため、書き込まないことがそのまま安全側の既定動作になります。

**既定値が必要になるのは、本人の同意は取得できていて result-report の選択だけ回答が得られない場合だけです。このとき `result_report_choice` の既定値は `保留` とします。** 「保留」は今回は提出しない扱いで、本人が後から `/setup` を再実行して「提出する」「提出しない」へ変更できます。`setup_completed_at` / `terms_sha256` / `data_policy_sha256` に既定値はありません（本人の同意と、その時点の実測ハッシュが必要です）。
...
- [ ] 「保留」は今回は提出しない扱いで、/setup 再実行で変更できると理解した
