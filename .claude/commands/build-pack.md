...
このコマンドは、申請書本文を作るものではありません。後続の `/draft-section` と `/review` が参照できる制度別の書き方メモを整える作業です。出力先は `input/spec/<subsidy_id>/`、参照先更新用の `input/current-application.json`、旧平置きファイルの退避先 `input/.archive/spec/<subsidy_id>/<spec-sha256>/` です。`.claude/commands/`、`schemas/`、`specs/`、`tools/`、`docs/` などのコア層は書き換えないでください。
...
この段階では参照先のずれを検出・案内するだけに留めます。次の共通案内による付け替えや再選択は、手順8の検査成功後に行ってください。本コマンドでの参照先更新は手順9のコードで行い、再解決の成功後に限り旧ファイルを退避します。
...
書き方メモの最終出力先は必ず `input/spec/<subsidy_id>/` です。解決済み spec が `specs/` または `input/spec/<subsidy_id>.json` にある場合は、顧客本人にコピーと検証後の旧平置きファイルの退避を説明して確認したうえで、spec と confirmation を `input/spec/<subsidy_id>/<subsidy_id>.json` と `input/spec/<subsidy_id>/<subsidy_id>.confirmation.json` にコピーしてください。コピー後の confirmation は、`spec_path` を新しい spec path に合わせます。`spec_sha256` はコピー後の spec bytes から再計算し、必要なら更新してください。コピー元の2ファイルは書き換えず保持します。同梱 `specs/` の原本は退避しません。

コピー前の解決済み spec の repo 相対パスを `<source_spec_path>` として控えてください。途中から再実行する場合も同じコピー元パスを使い、昇格後の解決結果で置き換えないでください。`input/current-application.json` の更新と旧ファイルの退避は手順9だけで行います。`state` と `chosen_funding` を含む既存項目は保持し、`spec_path` と変更時の `updated_at` だけを更新します。
...
repo のルートで、次のコードの `<subsidy_id>` と `<source_spec_path>` を実値に置き換えて実行してください。`check-pack` の成功を再確認し、参照先更新 → `--resolve-application` の成功と新パス一致 → 旧2ファイルの退避の順に進みます。再解決に失敗したら参照先を元に戻して停止します。どの失敗でも旧ファイルを削除だけで済ませないでください。

退避対象は、コピー元が `input/spec/<subsidy_id>.json` のときの旧 spec と旧 confirmation だけです。退避先は resolver の探索範囲外の `input/.archive/spec/<subsidy_id>/<spec-sha256>/` です。両ファイルを事前照合し、同名の退避ファイルは bytes が一致する場合だけ再利用します。不一致・欠損・シンボリックリンクは停止し、上書きしません。片方だけ退避済みでも同じコードで再開できます。

```bash
python3 - '<subsidy_id>' '<source_spec_path>' <<'PY'
import datetime
import hashlib
import json
import os
...
def plain_path(path):
    require(not any(p.is_symlink() for p in (path, *path.parents)),
            f"symbolic link is not supported: {path}")
    return path


def write_application(path, data):
    descriptor, name = tempfile.mkstemp(prefix=".build-pack-", dir=path.parent)
    try:
...
def promote():
    subsidy_id, source_text = sys.argv[1:]
    require(re.fullmatch(r"[a-z0-9-]+", subsidy_id), "invalid subsidy_id")
    source = pathlib.Path(source_text)
    flat = pathlib.Path(f"input/spec/{subsidy_id}.json")
    pack_dir = pathlib.Path(f"input/spec/{subsidy_id}")
    spec_path = plain_path(pack_dir / f"{subsidy_id}.json")
    require(source.parts and not source.is_absolute() and ".." not in source.parts
            and source.as_posix() == source_text
            and (source in (flat, spec_path) or source.parts[0] == "specs"),
            "use the original repo-relative source_spec_path")
    app_path = plain_path(pathlib.Path("input/current-application.json"))
    before = app_path.read_bytes()
    app = json.loads(before)
    require(isinstance(app, dict) and app.get("subsidy_id") == subsidy_id
            and isinstance(app.get("spec_path"), str) and app["spec_path"]
            and app.get("state") in ("spec_confirmed", "intake_done", "fit_done",
                                     "planned", "drafting", "verified", "finalized"),
            "current application must identify a confirmed spec")
    ...
    require(manifest["spec"]["path"] == spec_path.name
            and manifest["confirmation"]["path"] == f"{subsidy_id}.confirmation.json",
            "pack must list the promoted spec and confirmation paths")
    spec_bytes = spec_path.read_bytes()
    digest = hashlib.sha256(spec_bytes).hexdigest()
    archive = pathlib.Path(f"input/.archive/spec/{subsidy_id}/{digest}")
    records = []
    if source == flat:
        plain_path(archive)
        # Validate both files before changing references or creating archive entries.
        for old in (flat, flat.with_suffix(".confirmation.json")):
            plain_path(old)
            saved = plain_path(archive / old.name)
            require(old.is_file() or saved.is_file(), f"missing source and archive: {old}")
            data = old.read_bytes() if old.exists() else saved.read_bytes()
            if saved.exists():
                require(saved.read_bytes() == data, f"archive conflict: {saved}")
            records.append((old, saved, data))
        require(records[0][2] == spec_bytes, "original spec differs from verified pack")
        confirmation = json.loads(records[1][2])
        confirmation["spec_path"] = spec_path.as_posix()
        require(confirmation == json.loads(spec_path.with_suffix(".confirmation.json").read_bytes()),
                "original confirmation differs from verified pack")

    if app["spec_path"] != spec_path.as_posix():
        app["spec_path"] = spec_path.as_posix()
        app["updated_at"] = datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds")
        write_application(app_path, (json.dumps(app, ensure_ascii=False, indent=2) + "\n").encode("utf-8"))
    resolved = subprocess.run(["bash", "tools/check-spec.sh", "--resolve-application",
                               f"--subsidy-id={subsidy_id}"], stdout=subprocess.PIPE, text=True)
    if resolved.returncode != 0 or resolved.stdout.strip() != spec_path.as_posix():
        write_application(app_path, before)
        raise SystemExit("FAIL: promoted spec could not be resolved; application restored")

    if records:
        archive.mkdir(parents=True, exist_ok=True)
        # Exclusive links preserve bytes and refuse to replace an existing file.
        for old, saved, data in records:
            if not saved.exists():
                os.link(old, saved, follow_symlinks=False)
            require(saved.read_bytes() == data, f"archive bytes differ: {saved}")
        for old, saved, data in records:
            if old.exists():
                require(old.read_bytes() == saved.read_bytes() == data, f"source bytes changed: {old}")
                old.unlink()
        print(f"OK: original files archived in {archive}")
    print(f"OK: verified pack selected: {spec_path}")


try:
    promote()
except (OSError, ValueError, KeyError, TypeError, subprocess.CalledProcessError) as exc:
    raise SystemExit(f"FAIL: pack promotion stopped: {exc}")
PY
```

ファイルシステムがリンク作成に対応しない場合も旧ファイルを残して停止します。退避先の競合は内容を顧客本人と確認し、既存ファイルを上書き・削除せずに解消してください。

復元する場合は、表示された退避ディレクトリの `<subsidy_id>.json` と `<subsidy_id>.confirmation.json` を、同名の旧 `input/spec/` 直下へ bytes のままコピーします。コピーは排他的な新規作成（Python の `open("xb")` 等）で行い、既存ファイルがあれば一致を確認し、不一致なら停止してください。2ファイルそれぞれの bytes 一致を検証し、退避先も保持します。復元によって `state`・`chosen_funding`・参照先は変更しません。resolver は引き続きパック優先です。
