...
spec の `clauses[]` を読み、**spec の構造化フィールド（`deliverables[]` と `schedule[]`）のどこにも現れない申請者本人の工程**が条文に明記されている場合に限り、flow JSON に工程を追加します。

判断基準は「その工程が条文にしか書かれていないか」です。

- **deliverable として登録されている工程は AI 補完の対象外**です。提出前の準備でも提出後の手続きでも、derive が構造化フィールドから決定的に導出します。たとえば口頭審査が `deliverables[]` に `type: procedure` / `produced_by: human_only` で登録され、その `due_event_id` が申請締切より**後**の日付を持つイベントを指していれば、derive が提出と採択発表の間の独立工程として出します。手順2 の出力に既に在る工程を重ねて足さないでください。
- **申請締切と日付を比べられない手続きは、提出後のものでも derive では提出工程に吸収されます**（spec に申請締切イベントが無い、締切が相対期限だけで日付を持たない、その手続きに `due_event_id` が無い場合）。この場合も AI 補完で工程を足すのではなく、`/confirm-spec` に戻って schedule 側に期限イベントを持たせるのが正しい直し方です。
- **AI 補完の対象は、条文本文にだけ書かれていて deliverable にも schedule にもなっていない工程**です（例: 説明会への参加案内）。条文で確認できない工程は、構造化されていなくても追加できません。
