...
6. 選んだ spec に対して `bash tools/check-spec.sh <spec>` を必ず実行し、**終了コードが 0**（green）であることを確認してください。
...
評価入力は 2 文書に分かれます。`scope=profile` は `input/company-profile.json`、`scope=application` は `input/current-application.json` から読みます（この対応は subsidy-spec スキーマの `predicate.scope` の定義そのものです）。会社プロフィール側に `application` を埋め込んで渡してはいけません。同じ値が 2 箇所に分かれ、片方だけ更新した状態で評価が実態と食い違います。埋め込みがある場合、評価器は評価せずエラーで止まります。

`--current-application` は、評価する predicate が `scope=application` を 1 つでも参照する場合に必須です。渡さないと評価器は `unknown` を返さずエラーで止まります（配線ミスを「顧客が未確認」に偽装させないため）。`scope=profile` だけの predicate なら省略できます。`input/current-application.json` の `subsidy_id` と `spec_version` が評価対象 spec と食い違う場合も、別制度・別版の枠選択で評価しないようにエラーで止まります。

この時点では `chosen_funding` がまだ `null` のことがあります。その場合 `scope=application` の参照は `unknown` になります。`unknown` を都合よく `false` に変換せず、枠選択後に再評価してください。
