...
| 0D | 任意: `/journey-map` | confirmed spec から申請工程の参考図を作る | `input/journey/<subsidy_id>-journey.html` |
| 0E | `/build-pack` | confirmed spec から書き方メモを作る | `input/spec/<subsidy_id>/` の補助金パック |
...
`/journey-map` は、確認済み spec から、その補助金に固有の申請工程フローチャート（参考図 HTML）を `input/journey/` に生成する任意コマンドです。`bash tools/journey-map.sh derive` が spec の提出物・日程データだけから工程を機械的に導き、条文にしか書かれていない工程は AI が条文 ID の引用つきでのみ追加します。`bash tools/journey-map.sh render` は、スキーマ適合とすべての工程の出典（提出物・日程・条文）が spec に実在することを検証してから描画します。生成物は提出物ではなく全体像をつかむための参考図であり、日付・手続きの正は公募要領です。
...
**実値が不明なら数値キーは `null`** にします。上限確認だけで境界値を代入しないでください。たとえば「15 億円以下」と確認できても、課税所得の実額が `15` だとは限りません。確認済みの命題（有界事実）は、別の boolean 拡張キーに記録できます。

次は `input/company-profile.json` の該当部分だけを示す記録例です。`taxable_income_avg_3y_lte_15` は「確定している（申告済みの）直近過去3年分の各年または各事業年度の課税所得の年平均額（単位: 億円）が 15 以下」と定義します。全制度共通の必須キーではなく、対象期間・単位・比較条件は募集要項に合わせて確認してください。

```json
{
  "taxable_income_avg_3y": null,
  "taxable_income_avg_3y_lte_15": true
}
```

| boolean の値 | 記録する事実 |
| --- | --- |
| `true` | 命題を確認済み（この例では 15 億円以下） |
| `false` | 反対の事実を確認済み（この例では 15 億円を超える） |
| `null` | 未確認、または定義・出典が不足 |

未確認を `false` にしないでください。**以下（`lte`）と未満（`lt`）を区別**し、境界値ちょうどは「以下」に含め、「未満」には含めません。「15 億円以下」だけの確認では「15 億円未満」は未確認です。

`input/company-profile.md` の「根拠資料」には、キーごとに命題全文、比較演算子・境界値、単位、対象期間（対象年度・事業年度と起算点）、出典（募集要項の条文・ページと、事実を裏付ける資料名・該当箇所または顧客回答・回答日）、確認日を残します。実値の不足と命題の未確認を分け、未確認項目には `[要確認]` を付けてください。

スキーマに未宣言の拡張キーは許容されますが、spec が参照すると `/intake` で使うキー一覧では `declared=false` となり、`check-spec` は未宣言の会社情報キーを参照している旨の WARN を出します。定義・出典を確認し、既存キーへの改名や `false` への変換で消さないでください。キー名だけから期間・単位・条件が自動解釈されることはありません。

命題を機械照合するには、その拡張キーが `true` かを調べる条件を spec に明示する必要があります。**既存の数値条件は、実値が `null` なら `unknown` を維持**します。拡張キーが `true` でも、既存の条件式を自動で置換しないでください。`*_max` の自動解釈、区間演算、過去 profile の補正は行いません。
...
前回更新時の記録（`.update-core-state.json`）が repo に無い場合は、編集の有無を判定できないため `unknown-ancestor` として同じくスキップされます。この場合は `--force-file` を1件ずつ当てるのではなく、コアファイルを編集していない repo なら `bash tools/update-core.sh --generate-state`、編集している repo なら導入時バージョンの配布物を展開して `bash tools/update-core.sh --adopt-ancestor <導入時バージョンのパス>` を実行し、祖先を記録してから改めて dry-run と apply を行ってください。`--adopt-ancestor` は記録を書くだけで、コアファイルを書き換えません。自分の repo の update-core が古くこれらのオプションを持たない場合は、上流 checkout 側の update-core に `--repo-root .` を付けて自分の repo へ向けて実行してください（例: `bash ../saita-kun-planner-upstream/tools/update-core.sh --repo-root . --adopt-ancestor <導入時バージョンのパス>`）。続く `--dry-run` と `--apply` も同じ形で実行でき、その apply で update-core 自身も新しい版に更新されます。
