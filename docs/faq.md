...
WSL を使わない場合は、Git Bash + python3 を前提にしてください。Git Bash は Git for Windows に同梱されていますが、案内する配布元は公式の <https://git-scm.com/> のみにしてください。Python は `python3 --version`、`python --version`、Windows の場合は `py -3 --version` の順に確認します。`tools/check-*.sh` はこの 3 つを自動で試すため、どれかが Python 3.x を返せば wrapper は動きます。どれも返さない場合だけ、バージョン付きの名前（例: `python3.12`）や絶対パス（Git Bash なら `/c/Users/<名前>/AppData/Local/Programs/Python/Python313/python.exe`）で Python 3 を探し、`PLANNER_PYTHON` 環境変数に設定してください（例: `export PLANNER_PYTHON=python3.12`）。`PLANNER_PYTHON` に入れるのは実行ファイル 1 つだけで、`"py -3"` のように引数を付けた値は動きません。

ただし `PLANNER_PYTHON` が効くのは `tools/*.sh` の wrapper だけです。`/subsidy-fit`、`/verify`、`/finalize`、`/retrospect` などは `python3` を名前で直接呼ぶため、**`python3 --version` が Python 3.x を返す状態も必要**です。`python` や `py -3` しか無い環境では、それを `python3` という名前で PATH から呼べるようにしてください（例: `~/bin/python3` に `exec py -3 "$@"` の 1 行シムを置き、`PATH` に `~/bin` を通す）。`/setup` の環境セルフチェックは、`bash tools/check-spec.sh --list-bundled` の終了コードが 0 になることと `python3 --version` が通ることの 2 つを受入基準にしています。

どの方法でも、`/setup` で環境セルフチェックを行い、足りないものがあれば公式配布元または OS の公式手順で入れてから再実行してください。

## 許可を求められたら

Claude Code がコマンド実行の許可を求めることがあります。このキットで事前許可しているのは、`tools/check-*.sh`、`tools/draft-hash.sh`、条件判定用の同梱 Python スクリプトなど、キット同梱の検証スクリプトを動かすための read-only 検証系コマンドと、申請工程フローチャートを生成する `tools/journey-map.sh` だけです。

これらの検証は、募集要項 spec、叩き台、補助金パック、draft 本文ハッシュをローカルで確認するために使います。検証スクリプトは読み取り専用で、ファイル書き込みなし、ネットワーク送信なしの前提です。`tools/journey-map.sh` だけは生成コマンドで、書き込み先は `input/journey/` の生成物（工程データとフローチャート HTML）に限られ、こちらもネットワーク送信なしです。

拒否した場合、check が走らず検証が機能縮退します。これは静かに壊れた状態で進めないための仕組みなので、内容を確認したうえで許可することを推奨します。

`.claude/settings.json` が事前許可している範囲は 9 エントリです。対象は `python3 --version`、`pdftotext` の存在確認、`bash tools/check-spec.sh:*`、`bash tools/check-drafts.sh:*`、`bash tools/check-pack.sh:*`、`bash tools/check-quotes.sh:*`、`bash tools/draft-hash.sh:*`、条件判定用の同梱 Python スクリプト実行、`bash tools/journey-map.sh:*` に限られます。
...
すべて `unknown-ancestor` と表示されて何も取り込めない場合は、前回更新時の記録（`.update-core-state.json`）が repo に無い状態です。`--force-file` で1件ずつ当てず、コアファイルを編集していない repo なら `--generate-state`、編集している repo なら `--adopt-ancestor <導入時バージョンのパス>` で祖先を記録してから、もう一度 dry-run してください。これらのオプションが `unrecognized arguments` で失敗する場合は自分の repo の update-core が古い版です。上流 checkout 側の update-core に `--repo-root .` を付けて実行してください（例: `bash ../saita-kun-planner-upstream/tools/update-core.sh --repo-root . --adopt-ancestor <導入時バージョンのパス>`）。
...
## 境界線FAQ: 申請様式の「第三者からのアドバイス」欄に本キットの利用を書くべきか

補助金制度には「第三者からの支援・アドバイスの有無」を申告させる欄が設けられていることがあります。制度によっては、第三者の支援（支援料金の支払いの有無に関わらず）を受けているのに未記載の場合、虚偽の報告として不採択・交付決定取消の対象になると公募要領に明記されています（例: 小規模事業者持続化補助金の確認事項入力（様式2））。

本キット（無料の OSS テンプレート）と、あなた自身が契約・操作する AI の利用がこうした欄の開示対象に該当するかは、各制度側の解釈事項です。本キットは該当・非該当を判断しません。対象制度の公募要領・様式を確認し、判断に迷う場合は当該制度の事務局へ確認のうえ、回答に従って記載してください。確認した日時・回答内容をメモに残しておくと、後から説明が必要になったときに安心です。
