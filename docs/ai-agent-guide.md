...
4. **あなた自身の実行環境を判定してください**。判定したいのは「あなたが**利用者の端末上の、セッション終了後もファイルが残る永続ファイルシステム**でコマンドを実行できるか」です。自己認識で答えず、次の 2 つの観測で判定してください。

   - S1 で合意する clone 先の**親ディレクトリ**に対して `test -w '<親ディレクトリ>'` を実行し、終了コードが 0 であること（実在し、書き込みできること）。
   - そのパスが利用者自身の PC のものかを、利用者に 1 問で確認すること（例:「このパスは、あなたが普段 Claude Code を開いている PC のものですか？」）。

   両方を満たす場合のみ、以降の手順を自分で実行してかまいません（操作モード）。どちらか一方でも満たせない場合 — たとえばあなたが Web ブラウザ上のチャット AI である、またはクラウド上の一時的なサンドボックスで動いている場合 — は**代読案内モード**に切り替えてください。代読案内モードでは、あなたは手順を一段階ずつ読み上げて利用者に実行してもらう役に徹し、**自分の環境に clone しません**（一時環境に clone しても、利用者のローカルの Claude Code に引き渡せないためです）。
...
4. **clone 先の絶対パス**（例: macOS `/Users/<名前>/projects/my-subsidy-plan`、Linux / WSL `/home/<名前>/projects/my-subsidy-plan`、Windows の Git Bash `/c/Users/<名前>/projects/my-subsidy-plan`）。利用者の OS に合う形を使い、他 OS の形をそのまま提案しないでください。指定パスがまだ存在しないこと、親ディレクトリに書き込みできることを事前に確認してください。
