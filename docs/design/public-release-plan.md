...
> **対象：提供側（内部運用）**。利用者向けではありません。本書は公開準備フェーズの内部設計・実行計画であり、キットの使い方ではありません。利用者向けの入口は `docs/manual.md` と `docs/faq.md`、AI 向けの作業規範は `CLAUDE.md` です。
...
- パック方針（DRI 決定 2026-07-17 反映）: **4パック（持続化・ものづくり・IT導入・省力化）は第一目標の基礎スコープ**（未達含む最低履行義務。ストレッチ待ちにしない）。ストレッチゴールは**その先のパック追加**（国庫の主要補助金の追加 → 都道府県パック展開）で、金額が積むほど公共財が増える構図・リターン依存を避ける設計は従来どおり。詳細な段階表の正本: saita-kun `docs/plans/2026-07-17-crowdfunding-goal-return-usage-draft.md` §4。
...
3. **補助金パックの拡充** — 持続化 → ものづくり / IT導入 / 省力化（DRI 決定 2026-07-17 でクラファン第一目標の基礎スコープへ繰り上げ。ストレッチは国庫追加パック → 都道府県パック）
