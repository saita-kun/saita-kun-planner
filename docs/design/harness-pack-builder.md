...
- すべてのコマンド・検証系は spec / confirmation / notes の発見に上記の共通解決手順を使う（コマンド md には解決手順を同一文言で記載し、validate が文言を検査する）。同梱 spec の列挙・優先解決（パック優先・重複 ID FAIL・安定ソート）は `tools/lib/spec_resolver.py` を共通実装とし、コマンド台本からの呼出面は `bash tools/check-spec.sh --list-bundled` の出力を正とする（AI発見性設計 v4 §5-2 D-4。文章規約で同梱候補の探索順を再定義しない）。
...
- **AI 単独工程での例外（2026-08-26 追加）**: 原本を開ける人間（利用者本人・提供側のパック作成者）が同席しないヘッドレス実行では、この目視スポットチェックは実施できない。この場合に限り §7.3 の verbatim 機械照合をゲートとして代替してよい。ゲートの位置は抽出直後ではなく draft spec + confirmation 作成後（`/ingest-guidelines` 手順4）に移り、合格条件は「`check-spec.sh` の終了コード 0」「`matched` = `target` かつ `mismatched` = 0 かつ `skipped` = 0 かつ `target` = `clauses[]` 件数」「confirmation の READINESS 3 数の合計 = 列挙 item 件数」。不成立なら `state=spec_draft` に進まない。
- この代替が担うのは §7.3 と同じ「clause と extract の相互整合」までで、**原本そのものへの忠実性は担わない**。忠実性は突合（/confirm-spec）に一本化して回収する。人間が同席する実行では目視スポットチェックを省略しない（機械照合は上位互換ではない）。運用手順の正本は `.claude/commands/ingest-guidelines.md` 手順1 の「原本を開ける人間が同席しない実行での代替（機械照合）」。
