...
| --- | --- | --- | --- | --- | --- |
| jizokuka-20 | 第20回公募 | 2026-07-05〜2026-08-23（confirmed 68 / na 0） | 2026-08-23 | sha256 固定 1 / 全 2（live 再確認必須） | [中小企業庁 公募要領ページ](https://www.chusho.meti.go.jp/koukai/hojyokin/kobo/2026/260527002.html) |
...
一次情報取得先は既存の `source_documents[].url_or_path` の単一定義とし、別の URL フィールドや取得先リストへ転記しません。URL ならその取得先から原本を入手し、ローカルパスなら手元の原本を参照してください。

`portal_url` は公式の案内ページへの入口です。原本の取得先を確認するときに使います。たとえば `jizokuka-20` の `portal_url` は中小企業庁の第20回公募要領公開ページで、商工会議所地区と商工会地区の配布ページへ辿るための親ページです。

原本ファイルを入手したら、手元で SHA-256 を計算し、spec の `source_documents[].sha256` と一致するか確認してください。計算手段は自分の環境で使えるものを選びます。このキットの必須要件である Python 3 なら標準ライブラリだけで計算できます。

```bash
python3 -c 'import hashlib, pathlib, sys; print(hashlib.sha256(pathlib.Path(sys.argv[1]).read_bytes()).hexdigest())' <file>
```

OS 付属のコマンドを使う場合の例です（環境によってどちらか一方しかありません）。

```bash
shasum -a 256 <file>   # macOS など
sha256sum <file>       # 多くの Linux ディストリビューション
```
...
P-75 の共通 CLI（内部用の `tools/release/check-source-freshness.py`）は、`url_or_path` が HTTP(S) で `sha256` がある原本を照合します。既定実行は対象一覧のみで通信せず、`--fetch` で取得・照合します。一致（`match`）・不一致（`mismatch`）・未確認（`unverified`）を区別し、取得できない場合を一致扱いにしません。公開配布には含まれず、ローカル原本は照合対象外です。

`bash tools/validate.sh` はオフライン検査です。内部作業ツリーでは `python3 -B tools/release/test-source-freshness.py -v` により、現行 spec の取得情報と一致・不一致・未確認の契約を fake と一時ディレクトリで検証します。合格しても実際の原本の最新性を保証しません。

提供側の定期監視・定期起票は P-76 に集約し、P-75 の共通 CLI を使います。P-76 は DRI 確認待ち（deferred）です。この文書変更だけで鮮度監視の完了とはせず、issue #52 は P-76 の全 AC 合格まで close しません。P-77 の merge 時には「文書整備は完了・定期起票は P-76 待ち」と issue にコメントします。
