...
# Wave: tane04-section-definition-uniqueness
for checker in spec drafts; do
  check_contains "tools/lib/check_${checker}.py" "duplicate section_id:" "check-${checker} reports duplicate section definitions"
  check_contains "tools/test-check-${checker}.sh" "test_tane04_duplicate_section_definition" "check-${checker} test covers duplicate section definitions"
  check_contains "tools/test-check-${checker}.sh" "test_tane04_section_scope" "check-${checker} test allows section IDs in different deliverables"
done
check_contains "tools/test-check-drafts.sh" "test_tane04_preserve_first_definition" "check-drafts test preserves the first section definition"
...
check_contains ".claude/commands/finalize.md" "第三者支援の開示欄・誓約欄" "finalize checks third-party support disclosure"
check_contains ".claude/commands/finalize.md" "その様式と spec の clause が求める事項" "finalize checklist derives disclosure requirements from the form and spec"
check_contains ".claude/commands/ingest-guidelines.md" "produced_by=human_only" "ingest-guidelines keeps disclosure fields human-only"
check_contains ".claude/commands/ingest-guidelines.md" "開示・誓約まわりは二層で書き分ける" "ingest-guidelines documents the two-layer disclosure rule"
...
check_contains ".claude/commands/finalize.md" "final-checklist を次の 5 つに分けて作ってください" "finalize keeps five checklist groups"
check_contains ".claude/commands/finalize.md" "5. 条件付き成果物の該当確認" "finalize includes conditional deliverables group"
check_contains ".claude/commands/finalize.md" "## 条件付き成果物の該当確認" "finalize template includes conditional deliverables section"
check_contains ".claude/commands/finalize.md" '- [ ] 条件付き成果物（`required=false` かつ `required_if` が null/未設定）の該当有無を、顧客本人が1件ずつ「該当する」「該当しない」「未確定」で答えた' "finalize checklist confirms each conditional deliverable"
check_contains ".claude/commands/finalize.md" '- [ ] 「該当する」と答えた条件付き成果物を、AI と作る成果物・人がやること・添付資料のうち条件に当てはまる表すべてに追加して消し込み対象にした' "finalize checklist routes applicable conditional deliverables"
check_contains ".claude/commands/finalize.md" '条件付き成果物の「該当」欄が未記入のまま残っている場合は、その件数と `deliverable_id` を「提出直前に再確認すること」に必ず挙げてください。' "finalize reports unresolved conditional deliverables"
check_contains ".claude/commands/finalize.md" '「未確定」を正しい回答として認め' "finalize allows undetermined conditional applicability"
check_contains ".claude/commands/finalize.md" '1〜3 の各表の条件を**独立に**評価し、当てはまる表すべてに追加' "finalize routes conditional deliverables to every matching table"
...
for cmd in start select-subsidy ingest-guidelines confirm-spec journey-map intake subsidy-fit plan-deliverables draft-section review verify finalize retrospect; do
...
resolver_commands=(intake subsidy-fit plan-deliverables draft-section review verify finalize build-pack journey-map)
for cmd in "${resolver_commands[@]}"; do
  ...
  check_contains "$command_file" '`spec_path` が旧同梱平置きパス（例: `specs/<subsidy_id>.json`）を指し、同一 subsidy_id のパック形が上記の解決結果に存在する場合は、パック形を優先するため、`spec_path` を付け替えるか `/select-subsidy` で再選択するよう案内してください。' "resolver stale spec_path guidance for /${cmd}"
  check_contains "$command_file" '同梱 spec の候補を `specs/` の目視や推測で選ばないでください。' "resolver forbids manual bundled spec selection for /${cmd}"
done
...
sentinel_list_status=0
# Only ENOENT permits skipping; shell file predicates also hide access errors.
python3 - "$sentinel_file" <<'PY' || sentinel_list_status=$?
import os
import sys
...
try:
    ...
    print(f"public scrub sentinel scan execution failed: cannot stat word list: {error}",
          file=sys.stderr)
    raise SystemExit(1)
PY
if [ "$sentinel_list_status" -eq 0 ]; then
  # A subshell owns the temporary directory and its cleanup on every return.
  scan_public_scrub_sentinels() (
    sentinel_error() { echo "public scrub sentinel scan execution failed: $*" >&2; exit 1; }
    sentinel_words="$(cat "$sentinel_file")" || sentinel_error "cannot read word list"
    sentinel_tmp="$(mktemp -d "${TMPDIR:-/tmp}/public-scrub-sentinel.XXXXXX")" \
      || sentinel_error "cannot create temporary directory"
    trap 'rm -rf "$sentinel_tmp"' EXIT
    sentinel_files_file="$sentinel_tmp/tracked-files"
    sentinel_deleted_file="$sentinel_tmp/deleted-files"
    sentinel_hits_file="$sentinel_tmp/hits"
    git ls-files -z > "$sentinel_files_file" \
      || sentinel_error "cannot enumerate tracked files or write enumeration output"
    git ls-files -z --deleted > "$sentinel_deleted_file" \
      || sentinel_error "cannot enumerate deleted files or write enumeration output"
    # Git may also list inaccessible paths as deleted; verify actual absence.
    if ! python3 - "$sentinel_files_file" "$sentinel_deleted_file" \
      "${INTERNAL_ONLY_PATHS[@]}" <<'PY'
import os
import pathlib
import sys

tracked_path = pathlib.Path(sys.argv[1])
tracked_files = tracked_path.read_bytes().split(b"\0")
deleted_files = set()
# Match is_internal_only_path using the same normalized entries.
internal_paths = [os.fsencode(path) for path in sys.argv[3:]]
for path in pathlib.Path(sys.argv[2]).read_bytes().split(b"\0"):
    if not path or path.startswith(b".ralph/"):
        continue
    if any(path == entry or path.startswith(entry + b"/") for entry in internal_paths):
        continue
    try:
        os.lstat(path)
    except (FileNotFoundError, NotADirectoryError):
        deleted_files.add(path)
tracked_path.write_bytes(b"".join(
    path + b"\0" for path in tracked_files if path and path not in deleted_files
))
PY
    then
      sentinel_error "cannot filter deleted paths from tracked file list"
    fi
    # An empty record marks EOF explicitly; read errors must not look like EOF.
    printf '\0' >> "$sentinel_files_file" || sentinel_error "cannot finish enumeration output"
    sentinel_fail=0
    # Split the checked word-list contents in memory, without another input file.
    while [ -n "$sentinel_words" ]; do
      sentinel_word="${sentinel_words%%$'\n'*}"
      if [ "$sentinel_words" = "$sentinel_word" ]; then
        sentinel_words=""
      else
        sentinel_words="${sentinel_words#*$'\n'}"
      fi
      case "$sentinel_word" in ""|\#*) continue ;; esac
      # Open output separately: a redirection error can also return status 1.
      exec 3> "$sentinel_hits_file" || sentinel_error "cannot open scan output"
      sentinel_read_complete=0
      while IFS= read -r -d '' tracked_file; do
        if [ -z "$tracked_file" ]; then sentinel_read_complete=1; break; fi
        case "$tracked_file" in
          .ralph/*) ;;
          *)
            if ! is_internal_only_path "$tracked_file"; then
              if grep -Fn -- "$sentinel_word" "$tracked_file" >&3; then
                sentinel_fail=1
              else
                sentinel_status=$?
                [ "$sentinel_status" -eq 1 ] \
                  || sentinel_error "grep status $sentinel_status for $tracked_file"
              fi
            fi
            ;;
        esac
      done < "$sentinel_files_file" || sentinel_error "cannot read tracked file list"
      [ "$sentinel_read_complete" = "1" ] || sentinel_error "incomplete tracked file list read"
      exec 3>&-
      cat "$sentinel_hits_file" || sentinel_error "cannot read scan output"
    done
    [ "$sentinel_fail" = "0" ] || { echo "public scrub sentinel scan found non-public wording" >&2; exit 1; }
  )
  if scan_public_scrub_sentinels; then
    ok
  else
    bad "public scrub sentinel scan failed"
  fi
elif [ "$sentinel_list_status" -eq 3 ]; then
  ok
else
  bad "public scrub sentinel scan execution failed: cannot access word list"
fi

if [ "$VALIDATE_INTERNAL_MODE" = "1" ]; then
  # Wave: P-97 / TANE-39 sentinel scan execution failures.
  check_file "tools/release/test-sentinel-scan.sh" 1
  for test_name in test_tane39_grep_status test_tane39_enumeration_io_failure \
    test_tane39_unstaged_deleted_paths test_tane39_excluded_unreadable_directories \
    test_tane39_manifest_access_errors \
    test_tane39_word_list_access test_tane39_manifest_cache_exclusions test_tane39_shellcheck_literals \
    test_tane39_no_success_artifacts test_tane39_normal_and_public_tree; do
    check_in_order "tools/release/test-sentinel-scan.sh" "sentinel regression defines and runs $test_name" \
      "def $test_name():" "    $test_name()"
  done
  check_contains "tools/release/export.sh" 'sentinel scan execution failed' \
    "export diagnoses sentinel execution failures"
  # shellcheck disable=SC2016 # Assert literal shell source without expanding variables.
  check_contains "tools/validate.sh" 'git ls-files -z > "$sentinel_files_file"' \
    "sentinel enumeration uses checked output instead of process substitution"
  # shellcheck disable=SC2016 # Assert literal shell source without expanding variables.
  check_contains "tools/validate.sh" 'git ls-files -z --deleted > "$sentinel_deleted_file"' \
    "sentinel enumeration checks worktree deletions before scanning"
  check_contains "tools/release/test-sentinel-scan.sh" 'mode not in ("normal", "unstaged_delete", "unstaged_move")' \
    "sentinel regression keeps full validation for normal and unstaged worktrees"
  check_contains "tools/release/test-sentinel-scan.sh" 'and mode not in excluded_directory_modes' \
    "sentinel regression keeps full validation for excluded unreadable directories"
  check_contains "tools/test-update-core.sh" '"apply preserves seeded input data"' \
    "update-core regression preserves fixture input when source gitkeep is deleted"
...
check_contains "tools/lib/check_spec.py" "--list-predicate-keys" "check-spec exposes the predicate key registry"
check_contains "tools/lib/check_spec.py" "predicate references undeclared profile key" "check-spec warns on undeclared profile keys"
check_contains "schemas/company-profile.schema.json" "\"unit\"" "company profile schema declares units"
...
if PREDICATE_KEYS_JSON="$(
   ...
   printf '%s\n' "$PREDICATE_KEYS_JSON" |
     python3 -c 'import json, sys; value = json.load(sys.stdin); assert isinstance(value.get("keys"), list)' \
       >/dev/null 2>&1; then
  ok
else
  bad "tools/check-spec.sh --list-predicate-keys must return JSON for the bundled spec"
fi
unset PREDICATE_KEYS_JSON
...
for json_file in \
  ...
  tools/fixtures/profiles/unknown-emp.json \
  tools/fixtures/profiles/embedded-application.json; do
  check_file "$json_file" 1
  check_json "$json_file"
done
no_placeholder "tools/fixtures/profiles/embedded-application.json"
...
check_contains "tools/lib/predicate.py" "--current-application" "predicate evaluator takes current application explicitly"
check_contains "tools/lib/predicate.py" "application scope requires --current-application" "predicate evaluator fails when application input is missing"
check_contains "tools/lib/predicate.py" "profile must not contain top-level application" "predicate evaluator rejects embedded application context"
check_contains "tools/lib/predicate.py" "def normalize_args_for_parser" "predicate evaluator preserves leading-hyphen rule ids"
check_contains "tools/lib/predicate.py" "isinstance(value, bool)" "predicate binding rejects bool as spec version"
...
check_contains "tools/test-check-spec.sh" "addon-wage-up-chosen" "predicate CLI test covers explicit current application"
check_contains "tools/test-check-spec.sh" "nested-application-branch" "predicate CLI test covers nested application scope"
check_contains "tools/test-check-spec.sh" "LEADING_HYPHEN_PREDICATE_SPEC_FILE" "predicate CLI test covers leading-hyphen rule ids"
check_contains "tools/test-check-spec.sh" "current application spec_version must be an integer" "predicate CLI test covers binding type errors"
check_contains "tools/test-check-spec.sh" "chosen_funding.addon_ids" "predicate key registry test covers deliverable application keys"
check_contains "tools/test-check-spec.sh" "PREDICATE_KEYS_MERGE_SPEC_FILE" "predicate key registry test covers merged references"
check_contains "tools/test-check-spec.sh" "UNDECLARED_PROFILE_KEY_SPEC_FILE" "predicate key registry test covers undeclared profile warnings"
check_contains ".claude/commands/subsidy-fit.md" "--current-application" "subsidy-fit passes current application explicitly"
...
check_contains ".claude/commands/plan-deliverables.md" '| false | null または未設定 | 条件付き成果物（下記） | 「条件付き（該当する場合のみ提出）」に `[要確認]` 付きで並べる |' "plan-deliverables routes optional natural-language conditions"
check_contains ".claude/commands/plan-deliverables.md" '### `required=false` かつ `required_if` が null/未設定 の成果物（条件が自然文にしかない）' "plan-deliverables explains conditional deliverables"
check_contains ".claude/commands/plan-deliverables.md" "要約、言い換え、条件の追加をしないでください" "plan-deliverables preserves condition wording"
check_contains ".claude/commands/plan-deliverables.md" "該当するかどうかを AI が判定しません" "plan-deliverables leaves applicability to customer"
check_contains ".claude/commands/plan-deliverables.md" '`type` と `phase` は必ず表示してください' "plan-deliverables displays conditional type and phase"
check_contains ".claude/commands/plan-deliverables.md" '| 会社プロフィール（`scope=profile`） | 従業員数、所在地、業種による該当 | できる（推奨） |' "plan-deliverables allows profile predicates"
check_contains ".claude/commands/plan-deliverables.md" '| 今回の申請内容（`scope=application`） | 枠・加算・特例の選択（`chosen_funding.base` / `chosen_funding.addon_ids`）、資金調達の方法 | できる（推奨） |' "plan-deliverables allows application predicates"
check_contains ".claude/commands/plan-deliverables.md" '| 審査側や第三者の判断で事後に決まる | 「一定の基準を満たした事業者が対象」（口頭審査の対象化など） | できない。条件付きのまま自然文で残す |' "plan-deliverables rejects external judgment predicates"
check_contains ".claude/commands/plan-deliverables.md" "## 条件付き（該当する場合のみ提出）" "plan-deliverables template includes conditional section"
check_contains ".claude/commands/plan-deliverables.md" '- `required=false` かつ `required_if` が null/未設定 の成果物は、「条件付き（該当する場合のみ提出）」に全件並べ、条件の自然文を原文のまま転記して `[要確認]` を付けてください。該当有無を AI が判定しないでください。' "plan-deliverables output guard keeps conditional items"
check_in_order ".claude/commands/plan-deliverables.md" "plan-deliverables template orders conditional section" \
...
for raw in paths:
    ...
    seen.add(rel)
    path = root / rel
    # Explicit stat keeps access errors visible on Python 3.14 as well.
    try:
        regular_file = stat.S_ISREG(path.stat().st_mode)
    except FileNotFoundError:
        regular_file = False
    except OSError as error:
        errors.append(f"cannot stat core path: {rel}: {error}")
        continue
    if not regular_file:
...
def collect_core_files(directory):
    try:
        with os.scandir(str(directory)) as entries:
            for entry in entries:
                path = directory / entry.name
                rel = path.as_posix()
                # Prune internal paths before any stat or descent into the entry.
                if (is_internal_only(rel) or rel.startswith(".claude/commands/my-")
                        or "__pycache__" in path.parts):
                    continue
                try:
                    if entry.is_dir(follow_symlinks=False):
                        collect_core_files(path)
                    elif path.suffix != ".pyc" and stat.S_ISREG(path.stat().st_mode):
                        expected.add(rel)
                except FileNotFoundError:
                    # A path removed from the worktree is not a current file.
                    continue
                except OSError as error:
                    errors.append(f"cannot stat core candidate: {rel}: {error}")
    except FileNotFoundError:
        return
    except OSError as error:
        errors.append(f"cannot scan core directory: {directory}: {error}")

for base in (".claude/commands", "schemas", "specs", "tools", "templates", "docs"):
    collect_core_files(root / base)
expected.update({
    ".gitattributes",
    "AGENTS.md",
    "CLAUDE.md",
...
check_in_order "tools/test-update-core.sh" "manifest regression checks files inside pyc directories" \
...
check_contains "tools/update-core.sh" "--repo-root <dir>" "update-core documents an explicit repo root"
...
check_contains "tools/lib/update_core.py" "--generate-state" "update-core can generate export baseline state"
check_contains "tools/lib/update_core.py" "--adopt-ancestor" "update-core can adopt an installed-version ancestor"
check_contains "tools/lib/update_core.py" "unknown-ancestor" "update-core distinguishes missing ancestor state"
check_contains "tools/lib/update_core.py" "if value == STATE_FILE" "update-core rejects state file in core_paths"
...
check_contains "tools/test-update-core.sh" "SEEDED FIRST-RUN" "update-core test covers state-seeded first run"

if [ "$VALIDATE_INTERNAL_MODE" = "1" ]; then
  check_contains "tools/release/export.sh" "--generate-state" "export generates update-core state before commit"
  check_contains "tools/release/test-export.sh" ".update-core-state.json" "export test validates update-core state"
fi

if [ "$VALIDATE_INTERNAL_MODE" = "0" ]; then
  check_file ".update-core-state.json" 100
fi
if [ -f ".update-core-state.json" ]; then
  if python3 - <<'PY'
import json
import pathlib
import re
...
manifest = json.loads(pathlib.Path("core-manifest.json").read_text(encoding="utf-8"))
state_text = pathlib.Path(".update-core-state.json").read_text(encoding="utf-8")
if not state_text.endswith("\n"):
    raise SystemExit("state file must end with newline")
state = json.loads(state_text)
if state.get("state_version") != 1:
    raise SystemExit("state_version must be 1")
files = state.get("files")
if not isinstance(files, dict):
    raise SystemExit("files must be an object")
if list(files) != sorted(files):
    raise SystemExit("state file keys must be sorted")
core_paths = manifest.get("core_paths")
if not isinstance(core_paths, list) or not core_paths:
    raise SystemExit("core_paths must be a non-empty array")
if ".update-core-state.json" in core_paths:
    raise SystemExit(".update-core-state.json must not be in core_paths")
invalid_paths = []
for relative_path in files:
    ...
    if (
        ...
        or any(ch in relative_path for ch in "*?[]")
        or relative_path == ".update-core-state.json"
        or relative_path == "input"
        or relative_path.startswith("input/")
        or relative_path == "knowledge"
        or relative_path.startswith("knowledge/")
        or relative_path.startswith(".claude/commands/my-")
        or "/my-" in relative_path
    ):
        invalid_paths.append(relative_path)
if invalid_paths:
    raise SystemExit(f"invalid state paths: {invalid_paths}")
bad_hashes = [
    relative_path
...
if bad_hashes:
    raise SystemExit(f"invalid state sha256 entries: {bad_hashes}")
PY
  then
    ok
  else
    bad ".update-core-state.json format invalid"
  fi
fi
...
for application_fixture in \
  tools/fixtures/applications/chosen-funding-wage-up.json \
  tools/fixtures/applications/subsidy-id-mismatch.json \
  tools/fixtures/applications/spec-version-mismatch.json; do
  check_file "$application_fixture" 1
  check_json "$application_fixture"
  no_placeholder "$application_fixture"
done
...
check_contains ".claude/commands/intake.md" "--list-predicate-keys" "intake derives predicate keys from the spec mechanically"
check_contains ".claude/commands/intake.md" "目視の走査で代替したことにしない" "intake does not replace predicate key listing with visual inspection"
check_contains ".claude/commands/intake.md" '`confirm_phase` に `intake` を含む `scope=profile` のキー' "intake selects intake profile keys by confirm_phase"
...
# Wave: roadmap-current-subsidy-names
if [ "$(grep -cF 'ものづくり・商業・サービス生産性向上促進補助金、デジタル化・AI導入補助金（旧 IT導入補助金）' ROADMAP.md)" -eq 1 ]; then
  ok
else
  bad "roadmap-current-subsidy-names: expected names must appear exactly once"
fi
# Pin 1cd656d:ROADMAP.md with only the two subsidy names replaced.
# The fixed digest keeps exported templates independent of internal Git history.
if python3 - <<'PY' >/dev/null 2>&1; then
import hashlib
import pathlib

expected_sha256 = "64a7b026da2bbcdee8aff801cfdeee9cd83e861a1f24dd537abbc37b23475e95"
actual_sha256 = hashlib.sha256(pathlib.Path("ROADMAP.md").read_bytes()).hexdigest()
raise SystemExit(0 if actual_sha256 == expected_sha256 else 1)
PY
  ok
else
  bad "roadmap-name-only-change: ROADMAP.md must match the approved name-only replacement"
fi
...
check_contains ".claude/commands/ingest-guidelines.md" "原本を開ける人間が同席しない実行での代替（機械照合）" "ingest defines the AI-only machine-verification substitute"
check_contains ".claude/commands/ingest-guidelines.md" "機械照合は目視の上位互換ではなく" "ingest keeps the visual spot check mandatory when a human is present"
check_contains "docs/design/harness-pack-builder.md" "AI 単独工程での例外" "pack builder design records the AI-only spot-check exception"
check_contains ".claude/commands/ingest-guidelines.md" '表が中心の資料での extract 生成と `raw_text` / `text` の使い分け' "ingest documents table-heavy extract and raw_text/text split"
check_contains ".claude/commands/ingest-guidelines.md" '**`raw_text` を優先し、無ければ `text`**' "ingest states raw_text is the preferred verbatim needle"
check_contains ".claude/commands/ingest-guidelines.md" '`text` は読み順に正規化した verbatim です' "ingest defines text as reading-order normalized verbatim"
check_contains ".claude/commands/ingest-guidelines.md" '`pdftotext` の導入を顧客に要求しないでください' "ingest keeps pdftotext optional for customers"
...
check_contains "tools/lib/check_pack.py" "ATX_HEADING_RE" "check-pack excludes ATX headings from fact-like warning"
...
check_contains "tools/test-check-pack.sh" "heading-not-fact-like" "check-pack test covers heading exclusion"
check_contains "tools/test-check-pack.sh" "heading-like-but-not-heading" "check-pack test covers non-heading hash lines"
...
check_contains "$jizokuka_spec" '"spec_version": 5' "jizokuka bundled spec version is current"
check_contains "$jizokuka_spec" '"sha256": "661a3702fec6b214f5b77a8339d2db24f8ac1adeba5aa9ebb51cdcdfaedb01c7"' "jizokuka spec pins revision 8 guidelines sha256"
...
for clause_id in ("disclosure-001", "pledge-001"):
    matches = [clause for clause in clauses if clause.get("clause_id") == clause_id]
    if len(matches) != 1:
        raise SystemExit(1)
    clause = matches[0]
    if any(
        not isinstance(clause.get(field), str) or not clause[field]
        for field in ("text", "raw_text")
    ):
        raise SystemExit(1)
    if clause.get("page") != 2 or clause.get("source_document_id") != "koubo-r20":
        raise SystemExit(1)
PY
  ok
else
  bad "jizokuka spec must contain exactly one complete page-2 koubo-r20 disclosure-001 and pledge-001 clause"
fi
if python3 - "$jizokuka_spec" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
matches = [
    ...
    if clause.get("clause_id") == "disclosure-001"
]
if len(matches) != 1:
    raise SystemExit(1)
...
PY
  ...
  bad "jizokuka disclosure-001 must preserve the revision 8 core disclosure and penalty wording"
fi
if python3 - "$jizokuka_spec" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
matches = [
    ...
    if deliverable.get("deliverable_id") == "kakunin-jiko"
]
if len(matches) != 1:
    raise SystemExit(1)
deliverable = matches[0]
if deliverable.get("produced_by") != "human_only":
    raise SystemExit(1)
if deliverable.get("type") != "form_input" or deliverable.get("phase") != "application":
    raise SystemExit(1)
if not {"disclosure-001", "pledge-001"}.issubset(
    set(deliverable.get("source_clauses", []))
):
    raise SystemExit(1)
if not deliverable.get("depends_on"):
    raise SystemExit(1)
PY
  ok
else
  bad "jizokuka kakunin-jiko must remain a dependent human-only application form sourced from both disclosure clauses"
fi
if python3 - "$jizokuka_spec" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
clauses = [
    clause
    for clause in spec.get("clauses", [])
    if clause.get("clause_id") == "authorship-001"
]
if len(clauses) != 1:
    raise SystemExit(1)
clause = clauses[0]
if clause.get("page") != 2 or clause.get("source_document_id") != "koubo-r20":
    raise SystemExit(1)
if any(
    not isinstance(clause.get(field), str) or not clause[field].strip()
    for field in ("text", "raw_text")
):
    raise SystemExit(1)

deliverables = spec.get("deliverables", [])
for deliverable_id in ("keiei-keikaku", "hojo-jigyo-keikaku"):
    matches = [
        deliverable
        for deliverable in deliverables
        if deliverable.get("deliverable_id") == deliverable_id
    ]
    if len(matches) != 1 or "authorship-001" not in matches[0].get("source_clauses", []):
        raise SystemExit(1)
kakunin_jiko = [
    deliverable
    for deliverable in deliverables
...
if len(kakunin_jiko) != 1:
    raise SystemExit(1)
if "authorship-001" in kakunin_jiko[0].get("source_clauses", []):
    raise SystemExit(1)
PY
  ok
else
  bad "jizokuka authorship-001 must be a complete page-2 clause referenced by both plan deliverables and excluded from kakunin-jiko"
fi
if python3 - "$jizokuka_spec" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
spec = json.load(open(sys.argv[1], encoding="utf-8"))
required_boundary = "地域別最低賃金額未満"
legacy_boundary = "地域別最低賃金額以下"
clauses = [
    clause
    for clause in spec.get("clauses", [])
    if clause.get("clause_id") == "bonus-016"
]
bonus_items = [
    item
    for item in spec.get("bonus_items", [])
    if item.get("bonus_id") == "min-wage-raise-bonus"
]
if len(clauses) != 1 or len(bonus_items) != 1:
    raise SystemExit(1)
...
required_fields = (
    clause.get("text"),
    clause.get("raw_text"),
    bonus_item.get("description"),
)
if any(
    not isinstance(value, str) or required_boundary not in value
    for value in required_fields
):
    raise SystemExit(1)
if legacy_boundary in json.dumps(spec, ensure_ascii=False):
    raise SystemExit(1)
PY
  ok
else
  bad "jizokuka spec bonus-016 text/raw_text and min-wage-raise-bonus description must use the revision 8 boundary with no legacy inclusive wording anywhere"
fi
if python3 - "$jizokuka_spec" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
check_contains "$jizokuka_confirmation" '"spec_version": 5' "jizokuka confirmation spec version matches bundled spec"
if python3 - "$jizokuka_confirmation" <<'PY' >/dev/null 2>&1; then
import json
import sys
...
matches = [
    ...
    if item.get("field_path") == "deliverables.kakunin-jiko"
]
if len(matches) != 1 or matches[0].get("state") != "confirmed":
    raise SystemExit(1)
PY
  ok
else
  bad "jizokuka confirmation must confirm deliverables.kakunin-jiko"
fi
...
check_contains "$jizokuka_pack" '"spec_version": 5' "jizokuka pack spec version matches bundled spec"
...
check_contains "docs/faq.md" "9 エントリ" "FAQ pins permission allowlist count"
...
for setup_anchor in \
  ...
  'docs/data-policy.md' \
  'shasum -a 256' \
  'すべての確認項目が green で、利用者本人の利用規約への同意を確認した後にのみ' \
  '利用者本人から回答を得られない非対話実行（自動実行、CI、AI 単独実行など、本人がその場にいない実行）では、`input/setup-state.json` を書き込まないでください' \
  'setup-state を AI が自動生成して同意をスキップしない' \
  '既定値が必要になるのは、本人の同意は取得できていて result-report の選択だけ回答が得られない場合だけです。このとき `result_report_choice` の既定値は `保留` とします' \
  '「保留」は今回は提出しない扱い' \
  '`setup_completed_at` / `terms_sha256` / `data_policy_sha256` に既定値はありません' \
  '/setup 再実行で変更できると理解した'; do
...
standard = {
    ...
    "ingest-guidelines.md", "intake.md", "journey-map.md", "plan-deliverables.md",
    "retrospect.md", "review.md", "select-subsidy.md", "start.md", "subsidy-fit.md",
    "verify.md",
}
...
for f in \
  ...
  tools/fixtures/journey/schema-bad-flow.json; do
  check_file "$f" 1
  no_placeholder "$f"
done
...
check_contains ".claude/commands/journey-map.md" "preflight（setup ゲート）" "journey-map has setup gate preflight"
check_contains ".claude/commands/journey-map.md" "提出物ではありません" "journey-map states the map is not a deliverable"
check_contains ".claude/commands/journey-map.md" "clause_ids" "journey-map requires clause citations for ai steps"
check_contains ".claude/commands/journey-map.md" "行政書士法" "journey-map keeps the legal guardrail"
check_contains ".claude/commands/journey-map.md" "input/journey/" "journey-map writes only to input/journey"
check_contains ".claude/commands/journey-map.md" "spec / confirmation / notes の解決順" "journey-map documents shared resolver precedence"
check_contains ".claude/commands/journey-map.md" "bash tools/check-spec.sh <解決済みspec>" "journey-map requires check-spec green"
check_contains ".claude/commands/journey-map.md" "deliverable として登録されている工程は AI 補完の対象外" "journey-map excludes registered deliverables from ai completion"
check_contains ".claude/commands/journey-map.md" "AI 補完の対象は、条文本文にだけ書かれていて deliverable にも schedule にもなっていない工程" "journey-map limits ai completion to clause-only steps"

check_contains "tools/lib/spec_resolver.py" "def resolve_application_spec" "shared resolver exposes application spec resolution"
check_contains "tools/lib/journey_map.py" "spec_resolver.resolve_application_spec" "journey-map uses shared spec resolver"
check_contains "tools/lib/journey_map.py" "check_spec.check_spec(spec_path)" "journey-map runs normal check-spec validation"
check_contains "tools/lib/journey_map.py" "gate=\"confirm\"" "journey-map runs confirm gate validation"
check_contains "tools/test-journey-map.sh" "missing-required-key" "journey-map test covers missing required spec keys"
check_contains "tools/test-journey-map.sh" "stale confirmation sha" "journey-map test covers stale confirmation sha refusal"
check_contains "tools/test-journey-map.sh" "current application state must be spec_confirmed" "journey-map test covers unconfirmed state refusal"
check_contains "tools/test-journey-map.sh" "journey-stale" "journey-map test covers stale flat path resolver"

check_contains "docs/manual.md" "/journey-map" "manual mentions /journey-map"
check_contains "docs/faq.md" "bash tools/journey-map.sh:*" "FAQ lists the journey-map permission"
check_contains "docs/faq.md" "input/journey/" "FAQ scopes journey-map writes"
check_contains "CLAUDE.md" "/journey-map" "CLAUDE workflow references journey-map"

if bash tools/test-journey-map.sh >/dev/null 2>&1; then
  ok
else
  bad "tools/test-journey-map.sh failed"
fi
...
missing = sorted(required - members)
if missing:
    raise SystemExit(f"core manifest missing journey-map paths: {missing}")
PY
  ok
else
  bad "core manifest journey-map array membership failed"
fi

# Wave: application-spec-resolver-cli
check_contains "tools/lib/check_spec.py" 'parser.add_argument("--resolve-application", action="store_true")' "check-spec exposes application spec resolution"
check_contains "tools/lib/check_spec.py" '| --resolve-application [--subsidy-id ID]' "check-spec usage documents application spec resolution"
check_contains "tools/test-check-spec.sh" "--no-current-application" "check-spec resolver tests ignore ambient current application"
check_contains "tools/test-check-spec.sh" "run_isolated_check_spec" "check-spec resolver path tests use an isolated root"

# Wave: clause-quote-verifier
check_file "tools/check-quotes.sh" 700
check_file "tools/lib/check_quotes.py" 8000
check_file "tools/test-check-quotes.sh" 3000

check_contains "tools/test-check-quotes.sh" "literal_placeholder_values_verified" "quote tests verify literal values in clause text"
check_contains "tools/test-check-quotes.sh" "literal_na_id_verified" "quote tests resolve literal na IDs and reject unknown IDs"
check_contains "tools/test-check-quotes.sh" "legacy_empty_row_compatible" "quote tests preserve empty rows and resolve literal dash IDs"
check_contains "tools/test-check-quotes.sh" "empty_quote_rejected" "quote tests reject empty quotations for resolved IDs"

if bash tools/test-check-quotes.sh >/dev/null 2>&1; then
  ok
else
  bad "tools/test-check-quotes.sh must pass"
fi

check_contains ".claude/commands/review.md" "bash tools/check-quotes.sh" "review command runs the clause quote verifier"

no_placeholder "tools/check-quotes.sh"
no_placeholder "tools/lib/check_quotes.py"
no_placeholder "tools/test-check-quotes.sh"

# Wave: ingest-draft-check-stop
check_contains ".claude/commands/ingest-guidelines.md" \
  'draft の通常チェックが終了コード 0 になるまで、手順5および `/confirm-spec` の突合に進まないでください。' \
  "ingest-draft-check-stop requires a successful draft check before proceeding"

if grep -qF -- 'draft の通常チェックが green でなくても、`/confirm-spec` の突合に進めない場合があります。' \
  ".claude/commands/ingest-guidelines.md"; then
  bad "ingest-draft-check-stop must not contain ambiguous progression guidance"
else
  ok
fi
# Wave: python-resolver-minimum-version
check_file "tools/test-python-resolver.sh" 1000
for python_wrapper in check-spec.sh check-pack.sh check-drafts.sh check-quotes.sh journey-map.sh draft-hash.sh; do
  check_contains "tools/$python_wrapper" "sys.version_info >= (3, 7)" "wrapper requires Python 3.7+: $python_wrapper"
  check_contains "tools/test-python-resolver.sh" "\"$python_wrapper\"" "resolver regression covers $python_wrapper"
done
...
manifest = json.loads(pathlib.Path("core-manifest.json").read_text(encoding="utf-8"))
if manifest.get("core_paths", []).count("tools/test-python-resolver.sh") != 1:
    raise SystemExit("core manifest must include the Python resolver test exactly once")
PY
  ok
else
  bad "core manifest Python resolver test membership failed"
fi

if bash tools/test-python-resolver.sh; then
  ok
else
  bad "tools/test-python-resolver.sh must pass for all six wrappers"
fi
no_placeholder "tools/test-python-resolver.sh"

# Wave: P-89 / TANE-19 update path contract
check_contains "tools/lib/update_core.py" 'value != pure.as_posix() or not pure.parts' \
  "update-core rejects non-normalized manifest spelling and dot"
check_contains "tools/lib/update_core.py" 'check_protected_update_path(value)' \
  "update-core applies the shared protection check to manifest paths"
check_contains "tools/lib/update_core.py" 'check_protected_update_path(resolved_relative.as_posix())' \
  "update-core applies the shared protection check to resolved paths"
check_in_order "tools/lib/update_core.py" "update-core rechecks paths before apply writes" \
  'def apply_updates(' 'safe_join(repo_root, item.relpath)' \
  'local_path = safe_join(repo_root, item.relpath)' 'local_path.write_bytes(item.upstream_bytes)'
for test_name in test_tane19_manifest_normalization test_tane19_resolved_protected_paths \
  test_tane19_protected_hashes test_tane19_normal_updates; do
  check_contains "tools/test-update-core.sh" "def $test_name(" "update-core covers $test_name"
done

# Wave: P-92 / TANE-20 update snapshot and reclassification contract
check_in_order "tools/lib/update_core.py" "update-core hashes captured upstream bytes and keeps their metadata" \
  'with upstream_path.open("rb") as fh:' 'upstream_bytes = fh.read()' \
  'upstream_stat = os.fstat(fh.fileno())' 'upstream_hash = hashlib.sha256(upstream_bytes).hexdigest()' \
  'upstream_bytes=upstream_bytes' 'upstream_stat=upstream_stat'
check_in_order "tools/lib/update_core.py" "update-core reclassifies local content before reporting and writing" \
  'def apply_updates(' 'local_path = safe_join(repo_root, item.relpath)' \
  'local_hash = sha256_file(local_path)' \
  'status = classify_status(local_hash, item.upstream_hash, item.previous_hash)' \
  'if local_hash != item.local_hash:' 'status = "user-modified"' \
  'print(f"{status}\t{item.relpath}")' 'local_path.write_bytes(item.upstream_bytes)' \
  'next_state[item.relpath] = item.upstream_hash'
check_contains "tools/lib/update_core.py" 'local_path.chmod(stat.S_IMODE(item.upstream_stat.st_mode))' \
  "update-core preserves the captured file permissions"
for test_name in test_tane20_local_reclassification test_tane20_upstream_snapshot test_tane20_apply_contract; do
  check_contains "tools/test-update-core.sh" "def $test_name(" "update-core covers $test_name"
done
# P-92 V4: preserve captured file flags along with timestamps.
check_in_order "tools/lib/update_core.py" "update-core restores captured flags after content and timestamps" \
...
for test_name in test_tane20_file_metadata test_tane20_file_flags_errors; do
  check_contains "tools/test-update-core.sh" "def $test_name(" "update-core covers $test_name"
done
# P-92 V4: copy extended attributes before restoring captured metadata.
check_in_order "tools/lib/update_core.py" "update-core copies platform metadata after writing content" \
  'local_path.write_bytes(item.upstream_bytes)' \
  'write_xattrs(local_path, item.upstream_xattrs)' \
  'local_path.chmod(stat.S_IMODE(item.upstream_stat.st_mode))'
check_contains "tools/test-update-core.sh" 'def test_tane20_linux_extended_attributes(' \
  "update-core covers Linux extended attributes during normal apply"
# P-92 V4: capture extended attributes with the classified bytes for later apply.
check_in_order "tools/lib/update_core.py" "update-core captures attributes from the same open upstream file" \
  'with upstream_path.open("rb") as fh:' 'upstream_bytes = fh.read()' \
  'upstream_xattrs = read_xattrs(fh.fileno())' 'upstream_xattrs=upstream_xattrs'
for test_name in test_tane20_upstream_snapshot_during_apply test_tane20_xattr_error_contract; do
  check_contains "tools/test-update-core.sh" "def $test_name(" "update-core covers $test_name"
done
# Wave: tane01-json-object-roots
for checker in check_spec check_drafts check_pack; do
  check_contains "tools/lib/$checker.py" "root must be an object" "tane01 $checker diagnoses non-object JSON roots"
done
check_contains "tools/lib/journey_map.py" "current application root must be an object" "tane01 journey-map distinguishes absent and non-object applications"
check_in_order "tools/test-check-spec.sh" "tane01 spec/confirmation root regression is defined and run" \
  'test_tane01_spec_confirmation_roots() {' $'\ntest_tane01_spec_confirmation_roots\n'
for suite in check-drafts check-pack; do
  check_in_order "tools/test-$suite.sh" "tane01 $suite root regression is defined and run" \
    'test_tane01_draft_pack_roots() {' $'\ntest_tane01_draft_pack_roots\n'
done
check_in_order "tools/test-journey-map.sh" "tane01 application root regression is defined and run" \
  'test_tane01_application_roots() {' $'\ntest_tane01_application_roots\n'
# Wave: tane03-dependency-without-due
check_contains "tools/test-check-spec.sh" "test_tane03_missing_dependency_without_due" "check-spec tests unknown dependencies without a due event"
check_contains "tools/test-check-spec.sh" "test_tane03_valid_dependency_without_due" "check-spec tests valid dependencies without a due event"
check_contains "tools/test-check-spec.sh" "test_tane03_dependency_types" "check-spec tests dependency types without a due event"

# Wave: tane07-typed-comparisons
for test_name in test_tane07_comparison_matrix test_tane07_typed_membership test_tane07_existing_semantics; do
  check_contains "tools/test-check-spec.sh" "def $test_name(" "predicate regression suite includes $test_name"
done

if python3 -B - <<'PY'; then
from tools.lib.predicate import compare_values

cases = (
    (True, "lte", 5, "unknown"),
    (True, "in", [1], "unknown"),
    ([1], "contains", True, "unknown"),
    (False, "in", [0.0], "unknown"),
    ([True, 1.0], "contains", 1, "true"),
    (1, "in", [True, 2], "unknown"),
    (False, "in", [], "false"),
)
for actual, op, value, expected in cases:
    result = compare_values(actual, op, value)
    if result != expected:
        raise SystemExit(f"typed comparison mismatch: {actual!r} {op} {value!r}: {result} != {expected}")
PY
  ok
else
  bad "predicate comparisons must preserve boolean/number categories and three-valued membership"
fi

# Wave: P-75 internal source hash freshness
if [ "$VALIDATE_INTERNAL_MODE" = "1" ]; then
  check_file "tools/release/check-source-freshness.py"
  check_file "tools/release/test-source-freshness.py"
  if python3 -B tools/release/check-source-freshness.py --help >/dev/null; then
    ok
  else
    bad "source freshness CLI help must pass"
  fi
  if source_freshness_output="$(python3 -B tools/release/test-source-freshness.py -v 2>&1)"; then
    ok
  else
    printf '%s\n' "$source_freshness_output"
    bad "source freshness offline contract tests must pass"
  fi
fi
# Wave: tane05-confirmation-source-integrity
check_contains "tools/lib/check_spec.py" "def required_confirmation_sources(" "check-spec derives required source sets using confirmation field paths"
check_contains "tools/lib/check_spec.py" "return list(required_confirmation_sources(spec))" "check-spec shares required field paths with source coverage"
check_contains "tools/lib/check_spec.py" "required_sources = required_confirmation_sources(spec)" "check-spec connects source coverage to confirmation validation"
check_contains "tools/lib/check_spec.py" "unknown confirmation source_clauses reference" "check-spec diagnoses unknown confirmation sources"
check_contains "tools/lib/check_spec.py" "confirmation source_clauses mismatch" "check-spec diagnoses missing required confirmation sources"
for test_name in test_tane05_unknown_clause test_tane05_field_source_coverage \
  test_tane05_supplemental_sources test_tane05_source_types; do
  check_contains "tools/test-check-spec.sh" "def $test_name(" "confirmation source regression suite includes $test_name"
done
# Wave: P-84 retrospect record schema conformance
check_file "tools/test-retrospect.sh" 1000
for test_name in record_types_rejected record_patterns_and_nested_rules record_nullable_values_accepted; do
  check_in_order "tools/test-retrospect.sh" "retrospect covers $test_name" \
    "def $test_name():" "    $test_name()"
done
check_contains "core-manifest.json" '"tools/test-retrospect.sh"' "core manifest includes retrospect regression tests"
if bash tools/test-retrospect.sh; then
  ok
else
  bad "tools/test-retrospect.sh must pass the embedded record schema checks"
fi
no_placeholder "tools/test-retrospect.sh"
# Wave: P-78 ingest confirmation contract recheck
if python3 - <<'PY'; then
import json
import pathlib

schema = json.loads(pathlib.Path("schemas/spec-confirmation.schema.json").read_text(encoding="utf-8"))
assert "ingest_mode" not in schema["required"]
assert "extract_sha256" not in schema["required"]
assert set(schema["properties"]["ingest_mode"]["enum"]) == {"human_spot_check", "ai_only"}
hashes = schema["properties"]["extract_sha256"]
assert hashes["type"] == "object"
assert hashes["additionalProperties"] == {"type": "string", "pattern": "^[0-9a-f]{64}$"}
PY
  ok
else
  bad "confirmation schema must define optional ingest mode and extract byte hashes"
fi
check_in_order "tools/lib/check_spec.py" "check-spec loads the ingest contract before rechecking current extracts" \
  'def check_spec(' 'confirmation = check_confirmation_gateways(' 'verbatim_stats = verbatim_check('
check_in_order ".claude/commands/ingest-guidelines.md" "ingest records the selected mode and extract byte hashes before the draft gate" \
  '### 3.' '```bash' '"<ingest_mode>"' \
  'choices=("ai_only", "human_spot_check")' 'confirmation["ingest_mode"] = args.ingest_mode' \
  'for document in spec["source_documents"]:' 'hashlib.sha256(resolved.read_bytes()).hexdigest()' \
  'confirmation_path.write_text(' '### 4.' 'bash tools/check-spec.sh'
check_contains "tools/lib/check_spec.py" '手順 3 で ingest_mode と' "unmarked draft guidance points to the executable ingest recording step"
for test_name in ai_only_missing_extract_rejected ai_only_extract_hash_required \
  ai_only_incomplete_clause_coverage_rejected ai_only_unreferenced_document_missing_extract_rejected \
  ai_only_rechecked_after_restart unmarked_input_draft_rejected_with_reingest_guidance \
  unmarked_input_draft_links_require_reingest \
  unmarked_input_draft_case_variants_require_reingest \
  invalid_ingest_mode_rejected human_spot_check_compatible legacy_confirmed_pack_compatible \
  ai_only_valid_pack_accepted ingest_step3_ai_only_confirmation_passes_gate \
  ingest_step3_human_spot_check_confirmation_accepted ingest_step3_invalid_mode_preserves_confirmation; do
  check_contains "tools/test-check-spec.sh" "def test_$test_name(" "P-78 regression covers $test_name"
done
check_in_order "tools/test-check-spec.sh" "P-91/P-78 regression suites are defined and run" \
  'class ConfirmationSourceTests(unittest.TestCase):' \
  'class IngestContractTests(unittest.TestCase):' 'unittest.main(verbosity=2)' \
  'fail "P-91/P-78 confirmation contract checks must pass :: $confirmation_contract_output"'

# Wave: P-77 source URL and freshness responsibility contract
check_contains "specs/README.md" '一次情報取得先は既存の `source_documents[].url_or_path` の単一定義' \
  "source_url_contract uses the existing source locator as the single definition"
check_contains "specs/README.md" '`portal_url` は公式の案内ページへの入口' \
  "source_url_contract distinguishes the portal from the document locator"
check_contains "specs/README.md" 'P-75 の共通 CLI' \
  "source freshness uses the shared P-75 CLI"
check_contains "specs/README.md" '`bash tools/validate.sh` はオフライン検査' \
  "source freshness distinguishes offline validation from live confirmation"
check_contains "specs/README.md" 'python3 -B tools/release/test-source-freshness.py -v' \
  "source freshness documents the shared offline contract test"
check_contains "specs/README.md" '実際の原本の最新性を保証しません' \
  "source freshness keeps offline test success separate from current source freshness"
check_contains "specs/README.md" '提供側の定期監視・定期起票は P-76' \
  "source freshness delegates recurring monitoring to P-76"
check_contains "specs/README.md" 'P-76 は DRI 確認待ち（deferred）' \
  "source freshness records the deferred monitoring dependency"
check_contains "specs/README.md" 'issue #52 は P-76 の全 AC 合格まで close しません' \
  "source freshness keeps issue completion dependent on all P-76 criteria"
# Wave: P-120 Python 3.7/3.8 compatibility
check_file "tools/test-python-compat.sh"
check_contains "core-manifest.json" '"tools/test-python-compat.sh"' \
  "core manifest includes Python compatibility tests"
if python_compat_output="$(bash tools/test-python-compat.sh 2>&1)"; then
  ok
else
  printf '%s\n' "$python_compat_output"
  bad "tools/test-python-compat.sh must pass"
fi
no_placeholder "tools/test-python-compat.sh"
# Wave: P-83 optional outer pipes in quotation tables
check_contains "tools/test-check-quotes.sh" "table_outer_pipes_optional" "quote tests cover independent outer pipe styles"
check_contains "tools/test-check-quotes.sh" "table_outer_pipes_optional_inline_code" "quote tests distinguish inline code cells from code fences"
check_contains "tools/test-check-quotes.sh" "table_boundaries_respected" "quote tests cover table boundaries and non-table text"
check_contains "tools/test-check-quotes.sh" "table_boundaries_markdown_blocks.md" "quote tests end tables at lists, block quotes, and thematic breaks"
check_contains "tools/test-check-quotes.sh" "table_boundaries_list_dedent.md" "quote tests end list tables before unindented paragraphs"
check_contains "tools/test-check-quotes.sh" "table_boundaries_list_item_blocks.md" "quote tests respect block boundaries within containing list items"
check_contains "tools/test-check-quotes.sh" "table_boundaries_nested_list_dedent.md" "quote tests restore the outer list indentation after nested tables"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_in_list.md" "quote tests retain missing quotation checks inside lists"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_after_list.md" "quote tests retain missing quotation checks after lists"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_after_thematic_break.md" "quote tests distinguish thematic breaks from containing lists"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_after_empty_list.md" "quote tests retain missing quotation checks after empty list items"
check_contains "tools/test-check-quotes.sh" "table_boundaries_empty_list_dedent.md" "quote tests track the content indentation of empty list items"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_in_empty_list.md" "quote tests retain missing quotation checks in empty items with trailing spaces"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_after_paragraph.md" "quote tests distinguish paragraph continuations from containing lists"
check_contains "tools/test-check-quotes.sh" "table_boundaries_list_after_paragraph.md" "quote tests recognize lists interrupting paragraphs"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_in_list_paragraph.md" "quote tests retain the containing item through paragraph continuation"
check_contains "tools/test-check-quotes.sh" "table_boundaries_list_fences.md" "quote tests inspect tables after indented list fence closers"
check_contains "tools/test-check-quotes.sh" "table_boundaries_list_fence_dedent.md" "quote tests resume after a list containing an unclosed fence ends"
check_contains "tools/test-check-quotes.sh" "table_missing_quote_still_fails" "quote tests retain missing quotation checks for short rows"
check_contains "tools/test-check-quotes.sh" "escaped-pipes.md" "quote tests preserve escaped quotation pipes"
check_contains "tools/test-check-quotes.sh" "table_after_empty_item_blank.md" "quote tests close empty list items at the following blank line"
check_contains "tools/test-check-quotes.sh" "table_boundaries_lazy_continuation.md" "quote tests preserve list paragraphs through lazy continuation"
check_contains "tools/test-check-quotes.sh" "table_after_judgment_basis_list_fence.md" "quote tests track ordinary tables independently of quotation columns"
check_contains "tools/test-check-quotes.sh" "table_after_sibling_list_fence.md" "quote tests distinguish sibling ordered items from lazy paragraph continuation"
check_contains "tools/test-check-quotes.sh" "table_after_html_block_fence.md" "quote tests ignore fence markers and table rows within HTML blocks"
check_contains "tools/test-check-quotes.sh" "table_boundaries_html_comment.md" "quote tests end two-column tables before HTML comments"
check_contains "tools/test-check-quotes.sh" "table_after_list_html_block.md" "quote tests respect containing list boundaries for HTML blocks"
check_contains "tools/test-check-quotes.sh" "table_after_html_list_paragraph_dedent.md" "quote tests recognize standalone HTML blocks after a list paragraph dedent"
check_contains "tools/test-check-quotes.sh" "table_after_inline_html_list_paragraph.md" "quote tests preserve standalone tags continuing an indented paragraph"
check_contains "tools/test-check-quotes.sh" "table_after_html_inside_fence.md" "quote tests preserve fence context around HTML examples"
check_contains "tools/test-check-quotes.sh" "table_inline_html_cell.md" "quote tests preserve inline HTML quotation cells"

# Wave: P-83 unified Markdown block and differential contracts
check_contains "tools/lib/check_quotes.py" "def tokenize_markdown_lines(" "quote parsing uses one stateful line tokenizer"
check_contains "tools/lib/check_quotes.py" "for token in tokenize_markdown_lines(text):" "quotation collection consumes table tokens"
check_contains "tools/test-check-quotes.sh" "REVIEW_REGRESSIONS = (" "quote tests retain all four review rounds in one fixture matrix"
check_contains "tools/test-check-quotes.sh" '"r4_indented_code"' "quote tests distinguish code indentation relative to containers"
check_contains "tools/test-check-quotes.sh" '"r4_list_start_table"' "quote tests recognize tables on list opener lines"
check_contains "tools/test-check-quotes.sh" '"r4_quote_lazy_continuation"' "quote tests preserve block quote paragraph ownership"
check_file "tests/fixtures/check_quotes_legacy.py" 8000
check_contains "tools/test-check-quotes.sh" 'root / "tests/fixtures/check_quotes_legacy.py"' "quote differential uses the fixed legacy implementation"
check_contains "tools/test-check-quotes.sh" '"legacy_snapshot_sha256"' "quote differential pins the legacy snapshot bytes"
check_contains "tools/test-check-quotes.sh" '"git_ref_independent"' "quote differential passes without Git references"
check_contains "tools/test-check-quotes.sh" '"docs", "tests/fixtures", "specs", "tools/fixtures", "examples"' "quote differential discovers all review corpus roots"
check_contains "tools/test-check-quotes.sh" 'old[:2] == new[:2]' "quote differential compares exit status and verified count"
check_contains "tools/test-check-quotes.sh" "literal_placeholder_values_verified" "quote tests retain actual clause values"
check_contains "tools/test-check-quotes.sh" '"list_lazy_table_header"' "quote tests recognize table headers in lazy continuation"
check_contains "tools/test-check-quotes.sh" '"ordinary_table_short_separator"' "quote tests recognize GFM short table delimiters"
# Wave: tane09-required-if-draft-coverage
for test_name in test_tane09_required_matrix test_tane09_context_validation test_tane09_coverage_contract; do
  check_contains "tools/test-check-drafts.sh" "def $test_name(" "draft coverage regression suite includes $test_name"
done
for row in \
  '| true | null または未設定 | 今回の必須成果物 | 通常表示 |' \
  '| true / false | true | 今回の必須成果物 | 通常表示 |' \
  '| true / false | false | 今回の狙いでは対象外 |' \
  '| true / false | unknown | 提出漏れを避けるため必ず含める | 項目名と理由に `[要確認]` |' \
  '| false | null または未設定 | 条件付き成果物（下記） |'; do
  check_contains ".claude/commands/plan-deliverables.md" "$row" "draft coverage follows planning required_if table: $row"
done
check_contains "tools/lib/check_drafts.py" "predicate.eval_predicate(required_if, contexts)" "draft coverage uses the shared predicate evaluator"
check_contains "tools/lib/check_drafts.py" "predicate.validate_application_binding(spec, application_doc)" "draft coverage validates application binding"
for option in --profile --current-application; do
  check_contains "tools/lib/check_drafts.py" "parser.add_argument(\"$option\"" "draft checker accepts $option"
done
for doc in .claude/commands/verify.md docs/ハーネスの育て方.md; do
  check_contains "$doc" '--profile input/company-profile.json --current-application input/current-application.json' \
    "draft check example passes profile and application: $doc"
  if grep -qE '^bash tools/check-drafts\.sh .* --profile input/company-profile\.json --current-application input/current-application\.json$' "$doc"; then
    ok
  else
    bad "draft check invocation must pass profile and application: $doc"
  fi
done
check_contains ".claude/commands/verify.md" \
  '（bash tools/check-drafts.sh <spec> input/drafts/<subsidy_id>/ --profile input/company-profile.json --current-application input/current-application.json の出力）' \
  "verify report example records draft context arguments"

# Wave: P-121 pre-application dependency ordering
check_contains "tools/lib/journey_map.py" "def order_dependency_steps(" "journey-map shares dependency ordering"
check_contains "tools/lib/journey_map.py" "in a later placement region" "journey-map diagnoses reversed region dependencies"
for test_name in pre_application_dependency_order pre_application_dependency_cycle pre_application_dependency_boundaries; do
  check_contains "tools/test-journey-map.sh" "def test_$test_name(self):" "journey-map covers $test_name"
done
check_in_order "tools/test-journey-map.sh" "pre-application dependency regression is defined and run" \
  'test_pre_application_dependencies() {' 'unittest.main(argv=[sys.argv[0]])' $'\ntest_pre_application_dependencies\n'
# Wave: P-96 / TANE-06 manifest-selected confirmation validation
check_contains "tools/lib/check_spec.py" 'confirmation_path: typing.Optional[pathlib.Path] = None' \
  "tane06 spec checker accepts an optional confirmation path"
check_in_order "tools/lib/check_spec.py" "tane06 default confirmation is selected only when omitted" \
  'if confirmation_path is None:' 'confirmation_path = confirmation_path_for(spec_path)'
check_contains "tools/lib/check_pack.py" 'check_spec.check_spec(spec_path, confirmation_path=confirmation_path)' \
  "tane06 pack checker passes the selected confirmation to the shared checker"
check_in_order "tools/lib/check_pack.py" "tane06 pack resolves both listed files before spec validation" \
  'spec_file = read_listed_file(' 'confirmation_file = read_listed_file(' \
  'run_check_spec(spec_file[0], confirmation_file[0], errors)'
for test_name in test_tane06_named_confirmation test_tane06_named_confirmation_invalid; do
  check_in_order "tools/test-check-pack.sh" "tane06 $test_name is defined and run" \
    "$test_name() {" $'\n'"$test_name"$'\n'
done
check_in_order "tools/test-check-spec.sh" "tane06 confirmation selection regression is defined and run" \
  'test_tane06_confirmation_selection() {' $'\ntest_tane06_confirmation_selection\n'

# Wave: P-96 confirmation value type diagnostic preservation
check_in_order "tools/test-check-pack.sh" "tane06 invalid confirmation type regression is defined and run" \
  'test_tane06_confirmation_invalid_type_preserves_diagnostics() {' \
  $'\ntest_tane06_confirmation_invalid_type_preserves_diagnostics\n'

# Wave: P-96 input failure diagnostics and continued independent checks
check_in_order "tools/test-check-pack.sh" "tane06 failure matrix is defined and run" \
  'test_tane06_failure_matrix() {' $'\ntest_tane06_failure_matrix\n'
for failure_case in confirmation-type extract-missing extract-permission extract-decode confirmation-json \
  confirmation-missing confirmation-decode confirmation-sha-read note-decode note-permission spec-clauses-type \
  confirmation-clause-unknown confirmation-clause-mismatch; do
  check_contains "tools/test-check-pack.sh" "\"$failure_case\"" "tane06 failure matrix covers $failure_case"
done

# Wave: P-74 bounded fact recording contract
for bounded_fact_doc in .claude/commands/intake.md docs/manual.md; do
  check_contains "$bounded_fact_doc" '実値が不明なら数値キーは `null`' \
    "bounded_fact_contract preserves unknown exact values"
  check_contains "$bounded_fact_doc" '上限確認だけで境界値を代入しない' \
    "bounded_fact_contract forbids substituting a boundary for an exact value"
  check_contains "$bounded_fact_doc" 'taxable_income_avg_3y_lte_15' \
    "bounded_fact_contract names the separate boolean example"
  check_contains "$bounded_fact_doc" '反対の事実を確認済み' \
    "bounded_fact_contract defines a confirmed false proposition"
  check_contains "$bounded_fact_doc" '未確認を `false` にしない' \
    "bounded_fact_contract keeps unconfirmed propositions nullable"
  check_contains "$bounded_fact_doc" '以下（`lte`）と未満（`lt`）を区別' \
    "bounded_fact_contract distinguishes inclusive and exclusive bounds"
  check_contains "$bounded_fact_doc" '命題全文、比較演算子・境界値、単位、対象期間' \
    "bounded_fact_contract records the proposition and its measurement context"
  check_contains "$bounded_fact_doc" '出典（募集要項の条文・ページと、事実を裏付ける資料名・該当箇所または顧客回答・回答日）、確認日' \
    "bounded_fact_contract records definition and fact sources"
  check_contains "$bounded_fact_doc" 'declared=false' \
    "bounded_fact_contract explains undeclared extension keys"
  check_contains "$bounded_fact_doc" '既存の数値条件は、実値が `null` なら `unknown` を維持' \
    "bounded_fact_contract preserves unknown numeric predicates"
  check_contains "$bounded_fact_doc" '既存の条件式を自動で置換しない' \
    "bounded_fact_contract forbids automatic predicate replacement"
done
if python3 -B - <<'PY'
import json
import pathlib

schema = json.loads(pathlib.Path("schemas/company-profile.schema.json").read_text(encoding="utf-8"))
field = schema["properties"]["taxable_income_avg_3y"]
assert field["type"] == ["number", "null"]
assert field["unit"] == "億円"
assert "上限確認だけで境界値を代入しない" in field["description"]
assert schema["additionalProperties"] is True
assert "taxable_income_avg_3y_lte_15" not in schema["properties"]
assert "taxable_income_avg_3y_lte_15" not in schema["required"]
PY
then
  ok
else
  bad "bounded_fact_contract must preserve numeric types and optional extension keys"
fi
for bounded_fact_test in exact_value_unknown boolean_states boundary_operators undeclared_key; do
  check_in_order "tools/test-check-spec.sh" "bounded_fact_contract runs $bounded_fact_test regression" \
    'class BoundedFactTests(unittest.TestCase):' "def test_bounded_fact_${bounded_fact_test}(self):" \
    'unittest.main(verbosity=2)'
done

# Wave: P-73 verified pack promotion and recoverable flat-file archive
check_file "tools/test-build-pack-archive.sh" 1000
check_contains "core-manifest.json" '"tools/test-build-pack-archive.sh"' \
  "core manifest includes pack archive regression tests"
for test_name in archive_after_verified_pack archive_failure_preserves_sources \
  archive_retry_is_idempotent bundled_source_untouched archive_leading_hyphen_id; do
  check_contains "tools/test-build-pack-archive.sh" "def $test_name():" "pack archive covers $test_name"
done
check_in_order ".claude/commands/build-pack.md" "pack verification precedes reference update, resolution and archive" \
  '### 8. check-pack を green にする' \
  '### 9. 参照先を更新・再解決してから旧ファイルを退避する' \
  'subprocess.run(["bash", "tools/check-pack.sh", str(pack_dir)], check=True)' \
  'write_application(app_path, (json.dumps(app,' \
  'resolved = subprocess.run(["bash", "tools/check-spec.sh", "--resolve-application",' \
  'if resolved.returncode != 0 or resolved.stdout.strip() != spec_path.as_posix():' \
  'archive.mkdir(parents=True, exist_ok=True)' \
  'os.link(old, saved, follow_symlinks=False)' \
  'old.unlink()'
check_contains ".claude/commands/build-pack.md" 'input/.archive/spec/<subsidy_id>/<spec-sha256>/' \
  "pack archive stays outside resolver discovery"
check_contains ".claude/commands/build-pack.md" 'open("xb")' "pack archive documents exclusive restoration"
if archive_output="$(bash tools/test-build-pack-archive.sh 2>&1)"; then
  ok
else
  printf '%s\n' "$archive_output"
  bad "tools/test-build-pack-archive.sh must pass"
fi
no_placeholder "tools/test-build-pack-archive.sh"
