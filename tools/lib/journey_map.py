    ...
    if isinstance(schedule, list):
        for index, event in enumerate(schedule):
            ...
            if (
                ...
                errors.append(
                    "confirmed spec validation failed: "
                    f"$.schedule[{index}].relative.offset_days must be greater than "
                    "or equal to 0; correct the value and rerun /confirm-spec before "
                    "/journey-map"
                )
...
def is_post_submit_procedure(
    deliverable: Json,
    events_by_id: typing.Dict[str, Json],
    application_deadline_date: typing.Optional[str],
) -> bool:
    """Return whether a procedure has a comparable due date after submission.

    An unavailable application deadline or deliverable due date makes the
    boundary incomparable, so the procedure stays absorbed by kit-submit.
    """
    if (
        deliverable.get("produced_by") != "human_only"
        or deliverable.get("type") != "procedure"
        or deliverable.get("phase") != "application"
        or application_deadline_date is None
    ):
        return False
    due_event_id = deliverable.get("due_event_id")
    if not isinstance(due_event_id, str) or not due_event_id:
        return False
    due_event = events_by_id.get(due_event_id)
    if due_event is None:
        return False
    due_date = event_deadline_date(due_event)
    return due_date is not None and due_date > application_deadline_date


def event_deadline_source_dates(event: Json) -> typing.Tuple[str, ...]:
...
def relative_deadline_text(event: Json) -> typing.Optional[str]:
    relative = event.get("relative") if isinstance(event, dict) else None
    if not isinstance(relative, dict):
        return None
    anchor = relative.get("anchor")
    if not isinstance(anchor, str) or not anchor:
        return None
    offset_days = relative.get("offset_days")
    direction = relative.get("direction")
    if isinstance(offset_days, int) and not isinstance(offset_days, bool):
        if offset_days < 0:
            return None
        if direction == "within_after":
            return f"{anchor}から{offset_days}日以内"
        if direction == "before":
            return f"{anchor}の{offset_days}日前まで"
        return f"{anchor}起点・{offset_days}日（前後は要確認）"
...
def derived_step(
    ...
    errors: typing.List[str],
    *,
    deadline_note_support: bool = True,
) -> Json:
    ...
    if due_event_id:
        ...
        if event is None:
            ...
            elif deadline_note_support:
                deadline_note = relative_deadline_text(event)
                deadline_time = event_time(event)
    ...
    *,
    ...
    for field in ("ends_at", "date"):
        ...
        if isinstance(value, str) and value:
            ...
            break
    deadline_note = (
        relative_deadline_text(event)
        if deadline_note_support and deadline is None
        else None
    )
    return {
        ...
        "deadline": deadline,
        "deadline_note": deadline_note,
        "deadline_time": event_time(event) if deadline_note is not None else None,
        "issuer": None,
        "next": [],
        "branches": [],
    }
...
def displayed_event_ids(steps: typing.Iterable[Json]) -> typing.Set[str]:
    ...
    for step in steps:
        if (
            step["deadline"] is None
            and not step.get("deadline_note")
    ...
    for event_id in event_ids:
        ...
        milestones.append(
            absorbed_due_milestone(
                ...
                deadline_note_support=deadline_note_support,
            )
        )
...
def order_dependency_steps(
    entries: typing.List[typing.Tuple[str, Json]],
    deliverables_by_id: typing.Dict[str, Json],
    errors: typing.List[str],
    *,
    label: str = "post-adoption",
) -> typing.List[Json]:
    """Order steps deterministically within their placement region.

    Priority: supplied date key, then original spec order. A stable Kahn
    topological sort honors depends_on edges between represented deliverables.
    Use a constant date key when independent steps must retain spec order.
    """
    keyed = []
    ...
    for entry in keyed:
        for deliverable_id in entry["step"]["deliverable_ids"]:
            ...
            for dependency_id in deliverable.get("depends_on", []) or []:
                if dependency_id == deliverable_id:
                    errors.append(f"{label} deliverables have a depends_on cycle")
                    return []
...
def derive_flow_from_spec(
    ...
    *,
    ...
    submission_boundary_dates = []
    for event in schedule:
        if not isinstance(event, dict) or event.get("event_kind") != "application_deadline":
            continue
        deadline_date = event_deadline_date(event)
        if deadline_date is not None:
            submission_boundary_dates.append(deadline_date)
    # Use the latest comparable deadline so ambiguous procedures stay absorbed
    # by kit-submit (the existing fallback); only dates after every deadline
    # are conservatively classified as post-submit.
    submission_boundary_date = max(submission_boundary_dates, default=None)

    post_submit_ids = {
        deliverable_id
        for deliverable_id, deliverable in deliverables_by_id.items()
        if is_post_submit_procedure(deliverable, events_by_id, submission_boundary_date)
    }
    # Check individual deliverables before aggregation can hide their cycles.
    order_dependency_steps(
        [
            (DATE_MAX_KEY, {"deliverable_ids": [deliverable_id]})
            for deliverable_id, deliverable in deliverables_by_id.items()
            if deliverable.get("phase") == "application" and deliverable_id not in post_submit_ids
        ],
        deliverables_by_id,
        errors,
    ...
    post_submit_entries: typing.List[typing.Tuple[str, Json]] = []
    ...
    for deliverable in deliverables:
        ...
        if deliverable_id in post_submit_ids:
            due_event = events_by_id.get(deliverable.get("due_event_id"))
            date_key = (
                event_date_key(due_event)
                if isinstance(due_event, dict)
                else DATE_MAX_KEY
            )
            post_submit_entries.append(
                (
                    date_key,
                    derived_step(
                        deliverable,
    ...
    # Dependencies may point to the same or an earlier fixed placement region.
    pre_application_regions = [
        prep_steps, [steps_by_id["kit-draft"]], pre_final_steps, [steps_by_id["kit-submit"]],
    ]
    later_steps = [step for _, step in post_submit_entries + post_entries]
    region_by_deliverable = {
        deliverable_id: index
        for index, region in enumerate(pre_application_regions + [later_steps])
        for step in region
        for deliverable_id in step["deliverable_ids"]
    }
    for deliverable_id, region_index in region_by_deliverable.items():
        if region_index == len(pre_application_regions):
            continue
        for dependency_id in deliverables_by_id[deliverable_id].get("depends_on", []) or []:
            if region_by_deliverable.get(dependency_id, region_index) > region_index:
                errors.append(
                    f"pre-application deliverable {deliverable_id} depends_on {dependency_id} "
                    "in a later placement region"
                )
    if errors:
        return None
    ...
    if application_deadline is not None:
        ...
        if isinstance(date, str) and date:
            ...
            deadline_note = relative_deadline_text(application_deadline)
            if deadline_note is not None:
                submit["deadline_note"] = deadline_note
                time = event_time(application_deadline)
                if time is not None:
                    submit["deadline_time"] = time
                submit["event_ids"].append(application_deadline.get("event_id"))
    ...
    visibility_steps = (
        ...
        + prep_steps
        + pre_final_steps
        + [entry[1] for entry in post_submit_entries]
    ...
    prep_steps = order_dependency_steps(
        [(DATE_MAX_KEY, step) for step in prep_steps],
        deliverables_by_id, errors, label="pre-application",
    )
    pre_final_steps = order_dependency_steps(
        [(DATE_MAX_KEY, step) for step in pre_final_steps],
        deliverables_by_id, errors, label="pre-application",
    )
    if prep_steps:
    ...
    post_submit_steps = order_dependency_steps(
        post_submit_entries,
        deliverables_by_id,
        errors,
        label="post-submit",
    )
    insert_after_step(steps_by_id["kit-submit"], post_submit_steps)

    post_steps = order_dependency_steps(post_entries, deliverables_by_id, errors)
    if post_steps:
        chain(post_steps, [])
    ...
    for base_step in base["steps"]:
        ...
        if base_step["step_id"] == "submit":
            ordered_steps.extend(post_submit_steps)
        elif base_step["step_id"] == "env-setup":
    ...
    if not deadline_note_support:
        for step in ordered_steps:
            step.pop("deadline_note", None)

    flow = {"flow_version": 1}
    if deadline_note_support:
        flow["deadline_note_version"] = DEADLINE_NOTE_VERSION
    flow.update(
        {
...
def supports_deadline_notes(flow: Json) -> bool:
    return flow.get("deadline_note_version") == DEADLINE_NOTE_VERSION


def matching_relative_deadline_events(
    step: Json,
    events_by_id: typing.Dict[str, Json],
) -> typing.List[Json]:
    deadline_note = step.get("deadline_note")
    if not isinstance(deadline_note, str) or not deadline_note:
        return []
    matches: typing.List[Json] = []
    for event_id in step.get("event_ids", []):
        event = events_by_id.get(event_id)
        if (
            event is not None
            and event_deadline_date(event) is None
            and relative_deadline_text(event) == deadline_note
        ):
...
def validate_protected_flow(
    ...
    deadline_note_support = supports_deadline_notes(flow)
    ...
    for reference_step in reference["steps"]:
        ...
        for field in PROTECTED_STEP_FIELDS:
            if flow_step.get(field) != reference_step.get(field):
...
def validate_flow(
    ...
    steps = flow["steps"]
    deadline_note_support = supports_deadline_notes(flow)
    if not deadline_note_support and any(
        relative_deadline_text(event) is not None
        for event in spec.get("schedule", [])
        if isinstance(event, dict)
    ):
        errors.append(
            "this spec contains relative deadlines, so a legacy flow cannot be used; "
    ...
    for step in steps:
        ...
        dated_event_ids: typing.List[str] = []
        relative_deadline_event_ids: typing.List[str] = []
        if deadline_note_support:
            for event_id in step["event_ids"]:
                event = events_by_id.get(event_id)
                if event is None:
                    continue
                if event_deadline_source_dates(event):
                    dated_event_ids.append(event_id)
                elif relative_deadline_text(event) is not None:
                    relative_deadline_event_ids.append(event_id)
        mixes_deadline_formats = bool(dated_event_ids and relative_deadline_event_ids)
        if mixes_deadline_formats:
            errors.append(
                f"step {step_id} cannot mix dated event_ids {dated_event_ids!r} "
                "with relative deadline event_ids "
                f"{relative_deadline_event_ids!r}; use separate steps"
            )
        deadline_note = step.get("deadline_note")
        if step["deadline"] is not None and deadline_note is not None:
            errors.append(
                f"step {step_id} cannot set both deadline {step['deadline']!r} "
                f"and deadline_note {deadline_note!r}; use separate steps"
            )
        if step["deadline"] is not None:
            if not step["event_ids"]:
                ...
                for event_id in step["event_ids"]:
                    event = events_by_id.get(event_id)
                    if event:
                        dates.update(event_deadline_source_dates(event))
        ...
        if deadline_note is not None:
            if not step["event_ids"]:
                errors.append(f"step {step_id} has a deadline_note without a source event")
            else:
                relative_deadlines = set()
                for event_id in step["event_ids"]:
                    event = events_by_id.get(event_id)
                    if event is None:
                        continue
                    text = relative_deadline_text(event)
                    if text is not None:
                        relative_deadlines.add(text)
                if deadline_note not in relative_deadlines:
                    errors.append(
                        f"step {step_id} deadline_note {deadline_note!r} "
                        "does not match its events"
                    )
                matching_events = matching_relative_deadline_events(step, events_by_id)
                if matching_events:
                    expected_times = {event_time(event) for event in matching_events}
                    actual_time = step.get("deadline_time")
                    ...
                    if len(ordered_times) > 1:
                        errors.append(
                            f"step {step_id} deadline_note {deadline_note!r} has "
                            f"conflicting deadline_time values {ordered_times!r}; "
                            "use separate steps"
                        )
                    elif actual_time not in expected_times:
                        expected_text = repr(ordered_times[0])
                        errors.append(
                            f"step {step_id} deadline_time mismatch: "
                            f"expected {expected_text}, got {actual_time!r}"
                        )
        for event_id in step["event_ids"]:
            event = events_by_id.get(event_id)
            if not event:
            ...
            if displayed_deadline:
                if step["deadline"] is None:
                    errors.append(
                        f"step {step_id} references dated event {event_id} "
                        "without displaying a deadline"
                    )
            ...
            if not deadline_note_support or mixes_deadline_formats:
                continue
            displayed_relative_deadline = relative_deadline_text(event)
            if displayed_relative_deadline is None:
                continue
            if deadline_note is None:
                errors.append(
                    f"step {step_id} references relative deadline event {event_id} "
                    "without displaying deadline_note"
                )
            elif deadline_note != displayed_relative_deadline:
                errors.append(
                    f"step {step_id} displays deadline_note {deadline_note!r} but event "
                    f"{event_id} has relative deadline {displayed_relative_deadline!r}"
                )
        if step["kind"] == "decision":
...
def wrap_deadline_note(text: str) -> typing.List[str]:
    max_units = 21.0
    lines = wrap_text(text, max_units, max(1, len(text)))
    if len(lines) <= 2:
        return lines

    tail = ""
    tail_units = 1.0  # Reserve room for the leading ellipsis.
    for char in reversed(text):
        char_units = 0.5 if ord(char) < 128 else 1.0
        if tail and tail_units + char_units > max_units:
            break
        tail = char + tail
        tail_units += char_units
    return [lines[0], f"…{tail}"]
...
def render_svg(flow: Json) -> str:
    ...
    for step in ordered:
        ...
        elif step["deadline"] is None and step.get("deadline_note"):
            deadline_text = f"期限 {step['deadline_note']}"
            if step["deadline_time"]:
                deadline_text += f" {step['deadline_time']}"
            extra_lines.extend(
                (line, COLOR_DEADLINE) for line in wrap_deadline_note(deadline_text)
            )
        if is_gate:
