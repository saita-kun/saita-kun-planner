...
VALID_INGEST_MODES = {"human_spot_check", "ai_only"}
...
def check_schedule_relative_offsets(
    spec: typing.Dict[str, Json],
    errors: typing.List[str],
) -> None:
    schedule = spec.get("schedule") if isinstance(spec.get("schedule"), list) else []
    for index, event in enumerate(schedule):
        relative = event.get("relative") if isinstance(event, dict) else None
        if not isinstance(relative, dict):
            continue
        offset_days = relative.get("offset_days")
        if (
            isinstance(offset_days, int)
            and not isinstance(offset_days, bool)
            and offset_days < 0
        ):
            errors.append(
                f"$.schedule[{index}].relative.offset_days must be greater than "
                f"or equal to 0: {offset_days!r}"
            )
...
def collect_predicate_leaf_references(
    predicate: Json,
    referenced_by: str,
    references: typing.Dict[typing.Tuple[str, str], typing.Set[str]],
) -> None:
    if not isinstance(predicate, dict):
        return
    ...
    if "all" in predicate:
        ...
        if isinstance(items, list):
            for item in items:
                collect_predicate_leaf_references(item, referenced_by, references)
        return
    ...
    if "not" in predicate:
        collect_predicate_leaf_references(
            predicate.get("not"),
            referenced_by,
            references,
        )
    ...
    if (
        ...
        and scope in VALID_PREDICATE_SCOPES
        and isinstance(key, str)
        and key
    ):
        references.setdefault((scope, key), set()).add(referenced_by)


def collect_predicate_key_references(
    spec: typing.Dict[str, Json],
) -> typing.Dict[typing.Tuple[str, str], typing.Set[str]]:
    references: typing.Dict[typing.Tuple[str, str], typing.Set[str]] = {}
    eligibility = spec.get("eligibility") if isinstance(spec.get("eligibility"), dict) else {}
    rules = eligibility.get("rules") if isinstance(eligibility.get("rules"), list) else []
    for rule in rules:
    ...
    for deliverable in deliverables:
        ...
        if not isinstance(deliverable_id, str) or not deliverable_id:
            continue
        collect_predicate_leaf_references(
            deliverable.get("required_if"),
            f"deliverables.{deliverable_id}.required_if",
            references,
        )
    return references


def load_profile_schema_properties(
    root: pathlib.Path,
) -> typing.Optional[typing.Dict[str, Json]]:
    schema_errors: typing.List[str] = []
    schema = load_json(
        root / "schemas" / "company-profile.schema.json",
        schema_errors,
...
def predicate_key_registry(
    spec: typing.Dict[str, Json],
    root: pathlib.Path,
) -> typing.List[typing.Dict[str, Json]]:
    references = collect_predicate_key_references(spec)
    profile_properties = load_profile_schema_properties(root)
    registry: typing.List[typing.Dict[str, Json]] = []
    for scope, key in sorted(references):
        root_key = key.split(".", 1)[0]
        declared: typing.Optional[bool] = None
        property_schema: typing.Optional[typing.Dict[str, Json]] = None
        if scope == "profile" and profile_properties is not None:
            declared = root_key in profile_properties
            candidate = profile_properties.get(root_key)
            if declared and isinstance(candidate, dict):
                property_schema = candidate

        referenced_by = sorted(references[(scope, key)])
        confirm_phase: typing.Set[str] = set()
        for reference in referenced_by:
            if reference.startswith("eligibility.rules."):
                confirm_phase.add("intake")
            elif reference.startswith("deliverables."):
                confirm_phase.add("plan-deliverables")
        ...
        registry.append(
            {
                ...
                "root_key": root_key,
                "declared": declared,
                "type": property_schema.get("type") if property_schema is not None else None,
                "unit": property_schema.get("unit") if property_schema is not None else None,
                "description": (
                ...
                "confirm_phase": sorted(confirm_phase),
            }
        )
    return registry


def check_undeclared_profile_predicate_keys(
    spec: typing.Dict[str, Json],
    root: pathlib.Path,
    warnings: typing.List[str],
) -> None:
    for entry in predicate_key_registry(spec, root):
        if entry["scope"] != "profile" or entry["declared"] is not False:
            continue
        referenced_by = entry["referenced_by"]
        reference = referenced_by[0] if referenced_by else "unknown"
        warnings.append(
            "predicate references undeclared profile key: "
            f"{entry['key']} ({reference})"
        )


def required_confirmation_sources(spec: typing.Dict[str, Json]) -> typing.Dict[str, typing.Set[str]]:
    ...
    sources_by_path: typing.Dict[str, typing.Set[str]] = {}

    def add_sources(field_path: str, item: typing.Dict[str, Json]) -> None:
        sources = sources_by_path.setdefault(field_path, set())
        source_clauses = item.get("source_clauses")
        if isinstance(source_clauses, list):
            sources.update(clause_id for clause_id in source_clauses if isinstance(clause_id, str))

    schedule = spec.get("schedule") if isinstance(spec.get("schedule"), list) else []
    for event in schedule:
        if isinstance(event, dict) and isinstance(event.get("event_id"), str):
            add_sources(f"schedule.{event['event_id']}", event)

    eligibility = spec.get("eligibility") if isinstance(spec.get("eligibility"), dict) else {}
    rules = eligibility.get("rules") if isinstance(eligibility.get("rules"), list) else []
    for rule in rules:
        if isinstance(rule, dict) and isinstance(rule.get("rule_id"), str):
            add_sources(f"eligibility.rules.{rule['rule_id']}", rule)

    funding = spec.get("funding") if isinstance(spec.get("funding"), dict) else {}
    if isinstance(funding.get("base_award"), dict):
        add_sources("funding.base_award", funding["base_award"])
    ...
    for add_on in add_ons:
        if isinstance(add_on, dict) and isinstance(add_on.get("addon_id"), str):
            add_sources(f"funding.add_ons.{add_on['addon_id']}", add_on)
    ...
    for combination in combinations:
        if isinstance(combination, dict):
            ...
            if isinstance(addon_ids, list) and addon_ids and all(isinstance(addon_id, str) for addon_id in addon_ids):
                add_sources(f"funding.combinations.{'+'.join(addon_ids)}", combination)
    ...
    for expense in eligible_expenses:
        if isinstance(expense, dict) and isinstance(expense.get("category"), str):
            add_sources(f"funding.eligible_expenses.{expense['category']}", expense)

    bonus_items = spec.get("bonus_items") if isinstance(spec.get("bonus_items"), list) else []
    for bonus_item in bonus_items:
        if isinstance(bonus_item, dict) and isinstance(bonus_item.get("bonus_id"), str):
            add_sources(f"bonus_items.{bonus_item['bonus_id']}", bonus_item)
    ...
    for deliverable in deliverables:
        ...
        add_sources(f"deliverables.{deliverable_id}", deliverable)
        ...
        for section in sections:
            ...
            if section.get("max_chars") is not None:
                add_sources(f"deliverables.{deliverable_id}.sections.{section_id}.max_chars", section)
            if section.get("max_pages") is not None:
                add_sources(f"deliverables.{deliverable_id}.sections.{section_id}.max_pages", section)

    return sources_by_path


def required_confirmation_field_paths(spec: typing.Dict[str, Json]) -> typing.List[str]:
    return list(required_confirmation_sources(spec))
...
def is_input_spec_path(spec_path: pathlib.Path) -> bool:
    input_root = pathlib.Path(__file__).resolve().parents[2] / "input" / "spec"
    if not input_root.is_dir():
        return False
    # Resolve before lexical normalization to preserve directory links followed by "..".
    resolved = spec_path.resolve()
    path = pathlib.Path(os.path.abspath(spec_path))
    # Directory identity also preserves the filesystem's handling of name case.
    for parent in (*resolved.parents, *path.parents):
        try:
            if parent.samefile(input_root):
                return True
        except OSError:
            continue
    return False
...
def verbatim_check(
    ...
    errors: typing.List[str],
    warnings: typing.List[str],
    confirmation: typing.Optional[typing.Dict[str, Json]] = None,
) -> typing.Dict[str, int]:
    source_documents = spec.get("source_documents") if isinstance(spec.get("source_documents"), list) else []
    clauses = spec.get("clauses") if isinstance(spec.get("clauses"), list) else []
    ai_only = confirmation is not None and confirmation.get("ingest_mode") == "ai_only"
    extract_sha256 = (confirmation or {}).get("extract_sha256")
    if not isinstance(extract_sha256, dict):
        extract_sha256 = {}
    ...
    for index, document in enumerate(source_documents):
        ...
        if "extract_path" not in document or document.get("extract_path") is None:
            if ai_only:
                errors.append(f"ai_only requires extract_path: $.source_documents[{index}].extract_path")
            continue
        extract_path = document.get("extract_path")
        ...
        try:
            ...
            continue
        if ai_only:
            expected_sha = extract_sha256.get(document_id)
            if not isinstance(expected_sha, str) or not SHA256_RE.match(expected_sha):
                errors.append(f"ai_only requires a recorded extract_sha256 for document {document_id}")
            elif hashlib.sha256(raw_bytes).hexdigest() != expected_sha:
                errors.append(f"extract_sha256 mismatch for document {document_id}: {extract_path}")
    ...
    if ai_only and (
        stats["matched"] != stats["target"]
        or stats["target"] != len(clauses)
        or stats["mismatched"] != 0
        or stats["skipped_no_extract"] != 0
    ):
        errors.append(
            "ai_only requires complete verbatim coverage: "
            f"{stats['matched']}/{len(clauses)} clauses matched; "
            f"{stats['target']} targeted; {stats['mismatched']} mismatched; "
            f"{stats['skipped_no_extract']} skipped without extract_path"
        )
    return stats
...
def check_confirmation_structure(
    ...
    ingest_mode = confirmation.get("ingest_mode")
    if "ingest_mode" in confirmation and (
        not isinstance(ingest_mode, str) or ingest_mode not in VALID_INGEST_MODES
    ):
        errors.append("confirmation.ingest_mode must be human_spot_check or ai_only")

    if "extract_sha256" in confirmation:
        extract_sha256 = confirmation["extract_sha256"]
        if not isinstance(extract_sha256, dict):
            errors.append("confirmation.extract_sha256 must be an object keyed by document_id")
        else:
            for document_id, digest in extract_sha256.items():
                if not isinstance(digest, str) or not SHA256_RE.match(digest):
                    errors.append(f"confirmation.extract_sha256.{document_id} must be a 64-character lowercase hex string")
    ...
    elif not isinstance(confirmed_by, str) or confirmed_by not in VALID_CONFIRMED_BY:
    ...
    required_sources = required_confirmation_sources(spec)
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(source_clauses, list) or not source_clauses:
            ...
            actual_sources: typing.Set[str] = set()
            for clause_index, clause_id in enumerate(source_clauses):
                if not isinstance(clause_id, str):
                    ...
                    continue
                actual_sources.add(clause_id)
                if clause_id not in clause_ids:
                    errors.append(
                        "unknown confirmation source_clauses reference: "
                        f"{item_path}.source_clauses[{clause_index}]={clause_id} ({field_path})"
                    )
            if isinstance(field_path, str) and field_path in required_sources:
                missing_sources = required_sources[field_path] - actual_sources
                if missing_sources:
                    errors.append(
                        f"confirmation source_clauses mismatch: {item_path} field_path={field_path}; "
                        f"missing required references: {', '.join(sorted(missing_sources))}"
                    )

        state = item.get("state")
        if not isinstance(state, str) or state not in VALID_CONFIRMATION_STATES:
        ...
        if predicate_state is not None and (
            not isinstance(predicate_state, str) or predicate_state not in VALID_PREDICATE_STATES
        ):
        ...
        if confirmed_via is not None and (
            not isinstance(confirmed_via, str) or confirmed_via not in VALID_CONFIRMED_VIA
        ):
    ...
    for field_path in required_sources:
...
def check_confirmation_sha(
    ...
    try:
        ...
        errors.append(f"spec cannot be hashed: {spec_path}: {exc}")
        return
...
def check_reference_integrity(
    ...
    for index, deliverable in enumerate(deliverables):
        if not isinstance(deliverable, dict):
            continue
        seen_section_ids: typing.Set[str] = set()
        ...
        if isinstance(sections, list):
            for section in sections:
                ...
                if section_id in seen_section_ids:
                    errors.append(f"duplicate section_id: {deliverable.get('deliverable_id')}/{section_id}")
...
def check_confirmation_gateways(
    ...
    gate: typing.Optional[str],
    errors: typing.List[str],
    confirmation_path: typing.Optional[pathlib.Path] = None,
) -> typing.Optional[typing.Dict[str, Json]]:
    input_draft = spec.get("status") == "draft" and is_input_spec_path(spec_path)
    requires_confirmation = (
        confirmation_path is not None or spec.get("status") == "confirmed" or gate == "confirm" or input_draft
    )
    ...
    if not isinstance(confirmation, dict) and len(errors) == error_count:
        errors.append(f"confirmation root must be an object: {confirmation_path}")
    ingest_mode = confirmation.get("ingest_mode") if isinstance(confirmation, dict) else None
    if input_draft and (not isinstance(ingest_mode, str) or ingest_mode not in VALID_INGEST_MODES):
        errors.append(
            "confirmation.ingest_mode is required for input/spec draft; "
            "/ingest-guidelines を工程確認からやり直し、手順 3 で ingest_mode と "
            "extract_sha256（AI 単独時は全 source_documents 分）を記録してください"
        )
...
def check_spec(
    ...
    check_schedule_relative_offsets(spec, errors)
    check_predicates(spec, errors)
    check_undeclared_profile_predicate_keys(spec, root, warnings)
    ...
    confirmation = check_confirmation_gateways(spec, spec_path, gate, errors, confirmation_path=confirmation_path)
    verbatim_stats = verbatim_check(spec, spec_path, root, gate, errors, warnings, confirmation)
...
def load_resolution_current_application(
    root: pathlib.Path,
    raw_path: typing.Optional[str],
    no_current_application: bool,
    errors: typing.List[str],
) -> typing.Optional[typing.Dict[str, Json]]:
    ...
    if raw_path is None:
        path = root / "input" / "current-application.json"
        if not path.is_file():
            return None
    else:
    ...
    if len(errors) == error_count:
        errors.append(f"current application root must be an object: {path}")
    return None


def print_resolution_failures(messages: typing.Iterable[str]) -> None:
    # Command substitution consumes stdout, so diagnostics must not pollute the path line.
    for message in messages:
        print(f"FAIL: {message}", file=sys.stderr)


def resolve_application_cli(
    root: pathlib.Path,
    subsidy_id: typing.Optional[str],
    current_application_path: typing.Optional[str],
    no_current_application: bool,
) -> int:
    errors: typing.List[str] = []
    current_application = load_resolution_current_application(
        root,
    ...
    if errors:
        print_resolution_failures(errors)
        return 1

    entry_spec_path: typing.Optional[pathlib.Path] = None
    effective_subsidy_id = subsidy_id
    if current_application is not None:
        current_subsidy_id = current_application.get("subsidy_id")
        current_spec_path = current_application.get("spec_path")
        if not isinstance(current_subsidy_id, str) or not current_subsidy_id:
        ...
        if (
            subsidy_id is not None
            and isinstance(current_subsidy_id, str)
            and current_subsidy_id
            and subsidy_id != current_subsidy_id
        ):
            errors.append(
                "--subsidy-id mismatch with current application: "
                f"--subsidy-id={subsidy_id!r}, "
                f"current application subsidy_id={current_subsidy_id!r}"
            )
        if errors:
            print_resolution_failures(errors)
            return 1
        effective_subsidy_id = current_subsidy_id
        entry_spec_path = pathlib.Path(current_spec_path)

    if not isinstance(effective_subsidy_id, str) or not effective_subsidy_id:
        print_resolution_failures(
            ["subsidy_id is required; pass --subsidy-id <id>"]
        )
        return 1
    ...
    try:
        resolved = spec_resolver.resolve_application_spec(
            root,
            effective_subsidy_id,
            entry_spec_path,
        )
    except spec_resolver.ResolverError as exc:
        print_resolution_failures(exc.messages)
        return 1

    canonical_path = spec_resolver.repo_relative_path(resolved.spec_path, root)
    print(canonical_path)
    if resolved.stale_entry_path is not None:
        stale_path = spec_resolver.repo_relative_path(resolved.stale_entry_path, root)
        print(
            f"INFO: resolved canonical spec {canonical_path} instead of stale entry {stale_path}",
            file=sys.stderr,
        )
    return 0


def list_predicate_keys_cli(spec_path: pathlib.Path, root: pathlib.Path) -> int:
    errors: typing.List[str] = []
    spec = load_json(spec_path, errors, "spec")
    if not isinstance(spec, dict) and not errors:
        errors.append("spec root must be an object")
    if errors:
        for error in errors:
            print(f"FAIL: {error}")
    ...
    output = {
        ...
        "keys": predicate_key_registry(spec, root),
    }
...
def main(argv: typing.List[str]) -> int:
    parser = argparse.ArgumentParser(
        usage=(
            ...
            "| <spec.json> --list-predicate-keys "
            "| --list-bundled [--bundled-root DIR] "
            "| --resolve-application [--subsidy-id ID] "
            "[--current-application PATH | --no-current-application]"
        )
    )
    ...
    parser.add_argument("--list-predicate-keys", action="store_true")
    parser.add_argument("--list-bundled", action="store_true")
    parser.add_argument("--bundled-root", default=None)
    parser.add_argument("--resolve-application", action="store_true")
    parser.add_argument("--subsidy-id", default=None)
    current_application_group = parser.add_mutually_exclusive_group()
    current_application_group.add_argument("--current-application", default=None)
    current_application_group.add_argument(
    ...
    root = pathlib.Path(__file__).resolve().parents[2]
    if args.list_predicate_keys:
        if (
            args.gate is not None
            or args.now is not None
            or args.list_bundled
            or args.bundled_root is not None
            or args.resolve_application
            or args.subsidy_id is not None
            or args.current_application is not None
            or args.no_current_application
        ):
            parser.error(
                "--list-predicate-keys cannot be combined with --gate, --now, "
                "--list-bundled, --bundled-root, --resolve-application, "
                "--subsidy-id, --current-application, or --no-current-application"
            )
        if args.spec_path is None:
            parser.error("a spec path is required with --list-predicate-keys")
        return list_predicate_keys_cli(pathlib.Path(args.spec_path), root)

    if args.resolve_application:
        if (
            args.spec_path is not None
            or args.gate is not None
            or args.now is not None
            or args.list_bundled
            or args.bundled_root is not None
        ):
            parser.error(
                "--resolve-application cannot be combined with a spec path, --gate, "
                "--now, --list-bundled, or --bundled-root"
            )
        return resolve_application_cli(
            root,
            args.subsidy_id,
    ...
    if (
        args.subsidy_id is not None
        or args.current_application is not None
        or args.no_current_application
    ):
        parser.error(
            "--subsidy-id, --current-application, and --no-current-application "
    ...
    if args.list_bundled:
        ...
        bundled_root = pathlib.Path(args.bundled_root or "specs")
        try:
    ...
    if args.bundled_root is not None:
