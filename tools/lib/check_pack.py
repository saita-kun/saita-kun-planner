...
def load_json(path: pathlib.Path, errors: typing.List[str], label: str) -> Json:
    try:
        ...
        if not isinstance(value, dict):
            errors.append(f"{label} root must be an object: {display_path(path)}")
            return None
        return value
...
def check_pack_json_structure(pack: Json, errors: typing.List[str]) -> None:
    ...
    if not isinstance(pack.get("built_by"), str) or pack.get("built_by") not in VALID_BUILT_BY:
...
def check_note(
    ...
    if not isinstance(kind, str) or kind not in VALID_NOTE_KINDS:
...
def run_check_spec(
    ...
    confirmation_path: typing.Optional[pathlib.Path],
    errors: typing.List[str],
) -> None:
    try:
        spec_errors, _warnings, _readiness = check_spec.check_spec(spec_path, confirmation_path=confirmation_path)
    except TypeError:
        # Keep the former subprocess boundary for input failures in the shared checker.
        spec_errors = ["spec or confirmation contains an invalid value type"]
    except (OSError, ValueError) as exc:
        spec_errors = [f"spec or confirmation cannot be checked: {exc}"]
    if spec_errors:
...
def check_pack(pack_input: pathlib.Path) -> typing.Tuple[typing.List[str], typing.List[str]]:
    ...
    if spec_file is not None:
        if confirmation_file is not None:
            ...
            # Preserve spec checks and the selected path when a listed confirmation is unavailable.
            entry = pack.get("confirmation")
            pure = is_safe_relative_path(entry.get("path"), "confirmation", []) if isinstance(entry, dict) else None
            confirmation_path = resolve_pack_path(pack_dir, pure) if pure is not None else None
            run_check_spec(spec_file[0], confirmation_path, errors)
