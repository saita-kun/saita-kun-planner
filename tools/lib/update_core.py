...
def apply_updates(
    ...
    for item in statuses:
        ...
        if item.status == "unknown-ancestor" and item.relpath not in force_files:
            print(
        ...
        if item.status == "user-modified" and item.relpath not in force_files:
            ...
            continue
        if item.status in {"new", "changed", "unknown-ancestor", "user-modified"}:
